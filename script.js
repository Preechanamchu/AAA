document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        th: {
            ...
            menuManagerStore: "ผู้สมัครใช้ระบบ",
    ...
    const SUB_MENUS = {
        ...
        'manager-store': {
            'store-registrations': 'รายชื่อผู้สมัคร'
        }
    };
    ...
    document.getElementById('registration-form').addEventListener('submit', async (e) => {
        ...
        const formData = {
            ...
            packageType: 'standard', // Default package
    ...
    const ManagerStore = (() => {
        ...
        function renderStoreRegistrations() {
            ...
            console.log('⏳ Pending Registrations:', pendingList.length);
                ...
                const username = store.username || store.user_name || '-';

                return `
                <tr>
                    <td>${shopName}</td>
                    <td>${registeredAt ? new Date(registeredAt).toLocaleDateString('th-TH') : '-'}</td>
                    <td><code class="username-cell">${username}</code></td>
                    <td>
                        <span class="ms-status-badge pending">รอดำเนินการ</span>
                    </td>
                    <td>
                        <div class="action-btn-group">
                            <button class="ms-action-btn info" style="width: 100%;" onclick="ManagerStore.showRegistrationDetails(${store.id})">
                                👁️ ดูรายละเอียด
                            </button>
                        </div>
                    </td>
            ...
            `;
            }).join('') || '<tr><td colspan="5" class="ms-empty-state">📭 ไม่มีรายชื่อผู้สมัครรายใหม่</td></tr>';
        ...
        function renderRegistrationsHistory() {
            ...
            container.innerHTML = filtered.map(store => `
                <tr>
                    ...
                    <td>${store.username || store.user_name || '-'}</td>
        ...
        function showRegistrationDetails(storeId) {
            ...
            if (content) {
                content.innerHTML = `
                    <div class="registration-detail-container" style="padding: 10px; font-family: 'Kanit', sans-serif;">
                        <div class="detail-section" style="margin-bottom: 20px;">
                            <h3 style="color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">🏪 ข้อมูลร้านค้า</h3>
                            <p><strong>ชื่อร้านค้า:</strong> ${store.shopName || store.shop_name}</p>
                            <p><strong>อายุร้านค้า:</strong> ${store.shopAge || store.shop_age || '-'}</p>
                            <p><strong>ลิงก์ร้านค้า:</strong> ${store.shopLink || store.shop_link ? `<a href="${store.shopLink || store.shop_link}" target="_blank" style="color: blue; text-decoration: underline;">คลิกเพื่อดู</a>` : '-'}</p>
                        </div>

                        <div class="detail-section" style="margin-bottom: 20px;">
                            <h3 style="color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">👤 บัญชีผู้ใช้</h3>
                            <p><strong>Username:</strong> <code style="background: #f0f0f0; padding: 2px 5px; border-radius: 4px;">${store.username || store.user_name || '-'}</code></p>
                            <p><strong>Password:</strong> <span style="font-style: italic; color: #666;">(ซ่อนเพื่อความปลอดภัย)</span></p>
                        </div>

                        <div class="detail-section" style="margin-bottom: 20px;">
                            <h3 style="color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">📞 ช่องทางติดต่อ</h3>
                            <p><strong>LINE ID:</strong> ${store.contacts?.line || store.line_id || '-'}</p>
                            <p><strong>Facebook:</strong> ${store.contacts?.facebook || store.facebook_link ? `<a href="${store.contacts?.facebook || store.facebook_link}" target="_blank" style="color: blue; text-decoration: underline;">ลิงก์ Facebook/Messenger</a>` : '-'}</p>
                            <p><strong>เบอร์โทร:</strong> ${store.contacts?.phone || store.phone || '-'}</p>
                        </div>

                        <div class="detail-section">
                            <h3 style="color: var(--primary-color); border-bottom: 2px solid var(--primary-color); padding-bottom: 5px;">📅 ข้อมูลการสมัคร</h3>
                            <p><strong>วันที่สมัคร:</strong> ${new Date(store.registeredAt || store.registered_at).toLocaleString('th-TH')}</p>
                            <p><strong>สถานะ:</strong> <span class="status-badge ${store.status}">${store.status === 'pending' ? 'รอดำเนินการ' : 'อนุมัติแล้ว'}</span></p>
                        </div>
                    </div>
                `;
            }
