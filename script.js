document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        th: {
            ...
            closeBtn: "ปิด", cancelBtn: "ยกเลิก", confirmBtn: "ยืนยัน", saveBtn: "บันทึก", editBtn: "แก้ไข", deleteBtn: "ลบ", clearBtn: "ล้างข้อมูล", closeWindowBtn: "ปิดหน้าต่าง", proceedConfirmOrderBtn: "ยืนยันสั่งซื้อ >",
            ...
            manageCategoriesTitle: "จัดการหมวดหมู่", manageCoinPackagesTitle: "เติมเหรียญ", manageDiamondPassTitle: "เพชร & Pass", manageVoucherPackagesTitle: "บัตรกำนัล", categoryNameLabel: "ชื่อหมวดหมู่", categoryNameEnLabel: "ชื่อหมวดหมู่ (English)", categoryIconLabel: "ไอค่อนหมวดหมู่", minOrderLabel: "จำนวนสั่งซื้อขั้นต่ำ",
            ...
            orderSummaryTitle: "สรุปออเดอร์", copyOrderPrompt: "กดปุ่มคัดลอกออเดอร์เเละส่งให้ทางร้านได้เลย", copyOrderBtn: "คัดลอกออเดอร์", editOrderBtn: "แก้ไขออเดอร์", copySuccessMessage: "คัดลอกออเดอร์สำเร็จ",
        ...
        en: {
            ...
            closeBtn: "Close", cancelBtn: "Cancel", confirmBtn: "Confirm", saveBtn: "Save", editBtn: "Edit", deleteBtn: "Delete", clearBtn: "Clear", closeWindowBtn: "Close Window", proceedConfirmOrderBtn: "Confirm Order >",
            ...
            orderSummaryTitle: "Order Summary", copyOrderPrompt: "Please copy the text below to send to the shop.", copyOrderBtn: "Copy Order", editOrderBtn: "Edit Order", copySuccessMessage: "Order copied successfully",
    ...
    const SUB_MENUS = {
        ...
        'stock': {
            ...
            'coin-packages': 'manageCoinPackagesTitle',
            'diamond-pass': 'manageDiamondPassTitle',
    ...
    const loadCustomerData = async () => {
        try {
            ...
            console.log('📡 [loadCustomerData] Fetched shopSettings from API:', customerData.shopSettings);
    ...
    // ===== SMOOTH GRID SWITCH ANIMATION HELPER =====
    const animateGridSwitch = (renderFn) => {
        ...
        setTimeout(() => {
            // Phase 2: Render new content while hidden
            renderFn();
            // Phase 3: Fade in + slide up
            requestAnimationFrame(() => {
    ...
    const renderCategoryTabs = () => {
        ...
        appData.categories.forEach(cat => {
            ...
            tab.addEventListener('click', () => {
                logTrafficAction('category_click'); 
                ...
                // Use animation for smooth category switch
                animateGridSwitch(() => {
    ...
    window.getProductById = (productId) => {
        ...
        if(!product) {
            const coinPkg = (appData.shopSettings.coinPackages || []).find(p => p.id === productId);
            if(coinPkg) {
                return {
                    id: coinPkg.id,
                    name: `${coinPkg.coins.toLocaleString()} 💰`,
                    name_en: `${coinPkg.coins.toLocaleString()} 💰`,
                    level: coinPkg.levelRange,
                    price: coinPkg.price,
                    icon: coinPkg.icon || '',
                    category_id: 'coin_purchases',
                    isCoin: true
                };
            }
            // ===== DIAMOND PACKAGE LOOKUP =====
            const diamondPkg = (appData.shopSettings.diamondPackages || []).find(p => p.id === productId);
            if(diamondPkg) {
                return {
                    id: diamondPkg.id,
                    name: `${diamondPkg.diamonds.toLocaleString()} 💎`,
                    name_en: `${diamondPkg.diamonds.toLocaleString()} 💎`,
                    level: '',
                    price: diamondPkg.price,
                    icon: diamondPkg.icon || '',
                    category_id: 'diamond_purchases',
                    isDiamond: true
                };
            }
            // ===== FARM PASS PACKAGE LOOKUP =====
            const farmPassPkg = (appData.shopSettings.farmPassPackages || []).find(p => p.id === productId);
            if(farmPassPkg) {
                return {
                    id: farmPassPkg.id,
                    name: farmPassPkg.name,
                    name_en: farmPassPkg.name,
                    level: '',
                    price: farmPassPkg.price,
                    icon: farmPassPkg.image || '',
                    category_id: 'farmpass_purchases',
                    isFarmPass: true
                };
            }
            // ===== VOUCHER PACKAGE LOOKUP =====
            const voucherPkg = (appData.shopSettings.voucherPackages || []).find(p => p.id === productId);
            if(voucherPkg) {
                return {
                    id: voucherPkg.id,
                    name: `${voucherPkg.quantity.toLocaleString()} ใบ`,
                    name_en: `${voucherPkg.quantity.toLocaleString()} ใบ`,
                    level: '',
                    price: voucherPkg.price,
                    icon: voucherPkg.icon || '',
                    category_id: 'voucher_purchases',
                    isVoucher: true
                };
            }
        }
    ...
    const checkOrderValidation = () => {
        ...
        for (const productId in appData.cart) {
            ...
            if (quantity > 0) {
                ...
                if (product) {
                    ...
                    if (product.isCoin || product.isDiamond || product.isFarmPass || product.isVoucher) grandTotalPrice += (product.price * quantity);
                }
            }
        }
        ...
        for (const categoryId of sortedCategoryIds) {
            ...
            // ===== Skip min/max validation for virtual package categories =====
            if (categoryId === 'coin_purchases' || categoryId === 'diamond_purchases' || categoryId === 'farmpass_purchases' || categoryId === 'voucher_purchases') continue;
            // ===== END: Skip =====
    ...
    orderValidationMsg.addEventListener('click', (e) => {
        ...
        if (link) {
            ...
            // Use animation for smooth switch from validation link
            animateGridSwitch(() => {
                loadProductsForCategory(activeCategoryId);
            });
        }
    ...
    const createConfirmOrderSummary = (orderNumber) => {
                ...
                if (product) {
                    if (!itemsByCategory[product.category_id]) {
                        itemsByCategory[product.category_id] = {
                            items: [],
                            name: appData.categories.find(c => c.id === product.category_id)?.name || (product.isCoin ? 'แพ็กเกจเหรียญ' : product.isDiamond ? 'เติมเพชร' : product.isFarmPass ? 'Farm Pass' : product.isVoucher ? 'แพ็กเกจบัตรกำนัล' : 'Unknown'),
                            name_en: appData.categories.find(c => c.id === product.category_id)?.name_en || (product.isCoin ? 'Coin Packages' : product.isDiamond ? 'Diamonds' : product.isFarmPass ? 'Farm Pass' : product.isVoucher ? 'Vouchers' : 'Unknown')
                        };
                    }
        ...
        Object.keys(itemsByCategory).forEach(catId => {
            ...
            categoryData.items.forEach(item => {
                ...
                if (item.isCoin || item.isDiamond || item.isFarmPass || item.isVoucher) {
                    summaryText += `${prodName} - ${(item.price * item.quantity).toLocaleString()} ${currencySuffix}\n`;
                } else {
        ...
        for (const catId in itemsByCategory) {
            ...
            if (catId === 'coin_purchases') {
                const totalCoins = categoryData.items.reduce((sum, item) => {
                    const coinPkg = (appData.shopSettings.coinPackages || []).find(p => p.id === item.id);
                    return sum + ((coinPkg ? coinPkg.coins : 0) * item.quantity);
                }, 0);
                summaryText += `${catName}: ${totalCoins.toLocaleString()} 💰\n`;
            } else if (catId === 'diamond_purchases') {
                const totalDiamonds = categoryData.items.reduce((sum, item) => {
                    const dPkg = (appData.shopSettings.diamondPackages || []).find(p => p.id === item.id);
                    return sum + ((dPkg ? dPkg.diamonds : 0) * item.quantity);
                }, 0);
                summaryText += `${catName}: ${totalDiamonds.toLocaleString()} 💎\n`;
            } else if (catId === 'farmpass_purchases') {
                const fpItems = categoryData.items.map(item => item.name).join(', ');
                summaryText += `${catName}: ${fpItems}\n`;
            } else if (catId === 'voucher_purchases') {
                const totalVouchers = categoryData.items.reduce((sum, item) => {
                    const vPkg = (appData.shopSettings.voucherPackages || []).find(p => p.id === item.id);
                    return sum + ((vPkg ? vPkg.quantity : 0) * item.quantity);
                }, 0);
                summaryText += `${catName}: ${totalVouchers.toLocaleString()} ใบ\n`;
            } else {
            ...
            // ===== VIRTUAL PACKAGE PRICE: Use price directly instead of calculatePrice =====
            if (catId === 'coin_purchases' || catId === 'diamond_purchases' || catId === 'farmpass_purchases' || catId === 'voucher_purchases') {
                grandTotalPrice += categoryData.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
            } else {
                const priceResult = calculatePrice(parseInt(catId), totalQuantity);
                grandTotalPrice += priceResult.price;
            }
            // ===== END: VIRTUAL PACKAGE PRICE =====
        }
        summaryText += '\n';
    ...
    /**
     * Create HTML summary for visual display in confirmation modal
     * Includes product icons for better user experience
     */
    const createConfirmOrderHTML = (orderNumber) => {
        const lang = appData.shopSettings.language;
        const currencySuffix = lang === 'th' ? 'บาท' : 'THB';
        let html = "";
        ...
        html += '<hr style="border: 0.5px dashed var(--border-color); margin: 5px 0;">';

        Object.keys(itemsByCategory).forEach(catId => {
            const categoryData = itemsByCategory[catId];
            const catName = (lang === 'en' && categoryData.name_en) ? categoryData.name_en : categoryData.name;
            html += `<h4 class="order-summary-cat-title" style="margin-top: 8px; justify-content: center;">📋 ${catName}</h4>`;

            categoryData.items.sort((a, b) => a.level - b.level);

            categoryData.items.forEach(item => {
                const prodName = (lang === 'en' && item.name_en) ? item.name_en : item.name;
                const iconUrl = item.icon || 'https://placehold.co/100x100/e0e0e0/757575?text=?';
                
                // ===== Show price instead of quantity for virtual packages =====
                let quantityOrPrice = '';
                const isVirtual = item.isCoin || item.isDiamond || item.isFarmPass || item.isVoucher;
                if (isVirtual) {
                    quantityOrPrice = `<span class="order-item-quantity" style="color: var(--primary-color); font-weight: bold;">${(item.price * item.quantity).toLocaleString()} ${currencySuffix}</span>`;
                } else {
                    quantityOrPrice = `<span class="order-item-quantity">x ${item.quantity}</span>`;
                }
                // ===== END =====
                
                const namePrefix = isVirtual ? '' : 'LV';
                html += `
                    <div class="order-visual-item">
                        <div class="order-visual-info">
                            <img src="${iconUrl}" class="summary-item-icon" alt="${prodName}">
                            <span class="order-item-name">${namePrefix}${item.level} ${prodName}</span>
                        </div>
                        ${quantityOrPrice}
                    </div>`;
            });
        ...
        html += '<hr style="border: 0.5px dashed var(--border-color); margin: 15px 0;">';
        ...
        for (const catId in itemsByCategory) {
            ...
            if (catId === 'coin_purchases') {
                ...
                quantityDisplay = `${totalCoins.toLocaleString()} 💰`;
            } else if (catId === 'diamond_purchases') {
                const totalDiamonds = categoryData.items.reduce((sum, item) => {
                    const dPkg = (appData.shopSettings.diamondPackages || []).find(p => p.id === item.id);
                    return sum + ((dPkg ? dPkg.diamonds : 0) * item.quantity);
                }, 0);
                quantityDisplay = `${totalDiamonds.toLocaleString()} 💎`;
            } else if (catId === 'farmpass_purchases') {
                quantityDisplay = categoryData.items.map(item => item.name).join(', ');
            } else if (catId === 'voucher_purchases') {
                const totalVouchers = categoryData.items.reduce((sum, item) => {
                    const vPkg = (appData.shopSettings.voucherPackages || []).find(p => p.id === item.id);
                    return sum + ((vPkg ? vPkg.quantity : 0) * item.quantity);
                }, 0);
                quantityDisplay = `${totalVouchers.toLocaleString()} ใบ`;
            } else {
                quantityDisplay = `${totalQuantity} ${lang === 'th' ? 'ชิ้น' : 'pcs'}`;
            }
            // ===== END: QUANTITY DISPLAY =====
        ...
        if (currentAppliedPromo) {
            const discountAmount = grandTotalPrice * (currentAppliedPromo.discount / 100);
            finalTotal = grandTotalPrice - discountAmount;
            html += `<div class="summary-line" style="color: var(--danger-color); font-weight: bold; margin-top: 10px;">
                        <span>${translations[lang].discountLabel} (${currentAppliedPromo.code}):</span>
                        <span>-${discountAmount.toLocaleString(undefined, { minimumFractionDigits: 2 })} ${currencySuffix}</span>
                    </div>`;
        }
    ...
    const renderViewOrderModal = () => {
            ...
            if (quantity > 0) {
                ...
                if (product) {
                    if (!itemsByCategory[product.category_id]) {
                        ...
                        if (product.isCoin) fallbackName = lang === 'en' ? 'Coin Packages' : 'แพ็กเกจเหรียญ';
                        if (product.isDiamond) fallbackName = lang === 'en' ? 'Diamonds' : 'เติมเพชร';
                        if (product.isFarmPass) fallbackName = lang === 'en' ? 'Farm Pass' : 'Farm Pass';
                        if (product.isVoucher) fallbackName = lang === 'en' ? 'Vouchers' : 'บัตรกำนัล';
                        itemsByCategory[product.category_id] = {
                            items: [],
                            name: category ? ((lang === 'en' && category.name_en) ? category.name_en : category.name) : fallbackName,
            ...
            categoryData.items.forEach(item => {
                ...
                itemDiv.innerHTML = `
                    <div class="cart-item-info">
                        <img src="${iconUrl}" class="summary-item-icon" alt="${prodName}">
                        <span class="cart-item-name">${isVirtual ? '' : 'LV'}${item.level} ${prodName}</span>
                    </div>
                     <div class="cart-item-controls">
                        ${isVirtual ? `
                            <span style="font-weight: bold; color: var(--primary-color); margin-right: 15px;">${(item.price * item.quantity).toLocaleString()} บาท</span>
                        ` : `
        ...
        sortedCategoryIds.forEach(catId => {
            ...
            // ===== VIRTUAL QUANTITY DISPLAY =====
            if (catId === 'coin_purchases') {
                const totalCoins = categoryData.items.reduce((sum, item) => {
                    const coinPkg = (appData.shopSettings.coinPackages || []).find(p => p.id === item.id);
                    return sum + ((coinPkg ? coinPkg.coins : 0) * item.quantity);
                }, 0);
                summaryDiv.innerHTML += `<div class="summary-line"><span>${categoryData.name}:</span><span>${totalCoins.toLocaleString()} 💰</span></div>`;
            } else if (catId === 'diamond_purchases') {
                const totalDiamonds = categoryData.items.reduce((sum, item) => {
                    const dPkg = (appData.shopSettings.diamondPackages || []).find(p => p.id === item.id);
                    return sum + ((dPkg ? dPkg.diamonds : 0) * item.quantity);
                }, 0);
                summaryDiv.innerHTML += `<div class="summary-line"><span>${categoryData.name}:</span><span>${totalDiamonds.toLocaleString()} 💎</span></div>`;
            } else if (catId === 'farmpass_purchases') {
                const fpItems = categoryData.items.map(item => item.name).join(', ');
                summaryDiv.innerHTML += `<div class="summary-line"><span>${categoryData.name}:</span><span>${fpItems}</span></div>`;
            } else if (catId === 'voucher_purchases') {
                const totalVouchers = categoryData.items.reduce((sum, item) => {
                    const vPkg = (appData.shopSettings.voucherPackages || []).find(p => p.id === item.id);
                    return sum + ((vPkg ? vPkg.quantity : 0) * item.quantity);
                }, 0);
                summaryDiv.innerHTML += `<div class="summary-line"><span>${categoryData.name}:</span><span>${totalVouchers.toLocaleString()} ใบ</span></div>`;
            } else {
    ...
    cartDetails.addEventListener('click', (e) => {
        ...
        let productId = target.closest('[data-id]')?.dataset.id;
        if (!productId) return;
        
        // Ensure string IDs remain strings (coins, diamonds, farm pass, vouchers)
        if (!productId.startsWith('coin_') && !productId.startsWith('diamond_') && !productId.startsWith('farmpass_') && !productId.startsWith('voucher_')) {
        ...
        checkOrderValidation();
        renderProducts(searchBox.value.trim());
        // Re-render virtual catalogs if they are currently showing
        if (catalogPage === 1 && typeof renderCoinCatalogFrontend === 'function') {
            renderCoinCatalogFrontend();
        } else if (catalogPage === 2 && typeof renderDiamondPassCatalog === 'function') {
            renderDiamondPassCatalog();
        } else if (catalogPage === 3 && typeof renderVoucherCatalogFrontend === 'function') {
            renderVoucherCatalogFrontend();
        }
    });
    ...
    const handleOrderAction = (isConfirm) => {
        ...
        if (isConfirm) {
            ...
            // Render prominent badge
            const badge = document.getElementById('order-number-badge');
            if (badge) {
                const lang = appData.shopSettings.language;
                const label = lang === 'th' ? 'เลขที่ออเดอร์' : 'Order No.';
                badge.innerHTML = `<span class="badge-label">${label}:</span> <span class="badge-number">${orderNumber}</span>`;
                badge.style.display = 'flex';
            }

            orderDetails.innerHTML = createConfirmOrderHTML(orderNumber);
    ...
    let catalogPage = 0; // 0=products, 1=coins, 2=diamond/pass, 3=vouchers
    let swapRotation = 0;
    let isSwapping = false; // Guard against rapid clicks
    document.getElementById('swap-catalog-btn').addEventListener('click', function() {
        // ===== GUARD: Prevent rapid clicks causing duplicate/skipped pages =====
        if (isSwapping) return;
        isSwapping = true;
        // ===== END GUARD =====

        catalogPage = (catalogPage + 1) % 4;
        
        // ===== Spin animation for the button =====
        swapRotation += 360;
        this.style.transform = `rotate(${swapRotation}deg)`;
        // ===== END: Spin animation =====
        ...
        animateGridSwitch(() => {
            if (catalogPage === 1 || catalogPage === 2 || catalogPage === 3) {
                // ซ่อน controls wrapper สำหรับหน้า catalog
                if (controlsWrapper) {
                    controlsWrapper.style.transition = 'none';
                    controlsWrapper.style.opacity = '0';
                    controlsWrapper.style.maxHeight = '0';
                    controlsWrapper.style.overflow = 'hidden';
                ...
                if (catalogPage === 1) {
                    renderCoinCatalogFrontend();
                    document.getElementById('current-category-name').textContent = "เหรียญ";
                } else if (catalogPage === 2) {
                    if (typeof renderDiamondPassCatalog === 'function') renderDiamondPassCatalog();
                    document.getElementById('current-category-name').textContent = "เพชร & Farm Pass";
                } else if (catalogPage === 3) {
                    if (typeof renderVoucherCatalogFrontend === 'function') renderVoucherCatalogFrontend();
                    document.getElementById('current-category-name').textContent = "บัตรกำนัล";
                }
            } else {
                // คืนค่า controls wrapper สำหรับหน้าสินค้าปกติ
                if (controlsWrapper) {
                    controlsWrapper.style.transition = 'opacity 0.3s ease, max-height 0.4s ease';
                    controlsWrapper.style.opacity = '';
                    controlsWrapper.style.maxHeight = '';
                    controlsWrapper.style.overflow = '';
                    controlsWrapper.style.marginBottom = '';
                    controlsWrapper.style.padding = '';
                }
                
                const searchBox = document.getElementById('search-box');
                if (typeof renderProducts === 'function') {
                    renderProducts(searchBox ? searchBox.value.trim() : '');
                }
                
                const activeTab = document.querySelector('.category-tab.active');
                if (activeTab) {
                    document.getElementById('current-category-name').textContent = activeTab.textContent;
                } else if (appData.categories && appData.categories.length > 0) {
                    const lang = appData.shopSettings.language;
                    document.getElementById('current-category-name').textContent = lang === 'en' && appData.categories[0].name_en ? appData.categories[0].name_en : appData.categories[0].name;
                } else {
                    document.getElementById('current-category-name').textContent = translations[appData.shopSettings.language].itemsListTitle;
                }
            }

            // ===== UNLOCK: Allow next swap after render is done =====
            setTimeout(() => { isSwapping = false; }, 300);
        });
        // ===== END: SMOOTH TRANSITION =====
    });

    window.addCoinToCart = (id) => {
        const pkg = (appData.shopSettings.coinPackages || []).find(p => p.id === id);
        if(!pkg) return;
        
        // ===== SINGLE-SELECT: Clear all previous coin packages from cart =====
        const allCoinIds = (appData.shopSettings.coinPackages || []).map(p => p.id);
        allCoinIds.forEach(coinId => {
            if (appData.cart[coinId]) delete appData.cart[coinId];
        });
        
        // Add only the newly selected package (quantity = 1)
        appData.cart[id] = 1;
        // ===== END: SINGLE-SELECT =====
        
        localStorage.setItem('warishayday_cart', JSON.stringify(appData.cart));
        checkOrderValidation();
        renderCoinCatalogFrontend(); // Re-render to show checkmark
        Notify.success('เลือกแพ็กเกจเหรียญแล้ว', `${pkg.coins.toLocaleString()} 💰 ถูกเพิ่มลงบัญชีสั่งซื้อ`);
    };

    window.renderCoinCatalogFrontend = () => {
        const productGrid = document.getElementById('product-grid');
        if (!productGrid) return;
        productGrid.innerHTML = '';
        
        const packages = appData.shopSettings.coinPackages || [];
        if(packages.length === 0) {
            productGrid.innerHTML = `
                <div style="width: 100%; min-height: 400px; background-color: #ffffff; display: flex; align-items: center; justify-content: center; flex-direction: column; grid-column: 1 / -1; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <span style="font-size: 40px; color: #ccc; margin-bottom: 10px;">📦</span>
                    <span style="color: #888; font-family: var(--global-font-family); font-size: 1.2rem;">ยังไม่มีแพ็กเกจเหรียญ...</span>
                </div>
            `;
            return;
        }

        // ===== SINGLE-SELECT: Determine which coin package is currently selected =====
        const allCoinIds = packages.map(p => p.id);
        const selectedCoinId = allCoinIds.find(coinId => appData.cart[coinId] && appData.cart[coinId] > 0) || null;
        // ===== END: SINGLE-SELECT =====

        const levelGroups = {};
        packages.forEach(pkg => {
            if(!levelGroups[pkg.levelRange]) levelGroups[pkg.levelRange] = [];
            levelGroups[pkg.levelRange].push(pkg);
        });

        const container = document.createElement('div');
        container.className = 'coin-catalog-container';
        
        // CSS rules injected or inline. Inline to match current style approach quickly.
        container.style.cssText = 'width: 100%; grid-column: 1/-1; display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px;';

        for(const [level, pkgs] of Object.entries(levelGroups)) {
            pkgs.sort((a,b) => a.coins - b.coins);
            
            const groupDiv = document.createElement('div');
            groupDiv.className = 'coin-level-group';
            groupDiv.innerHTML = `
                <h3 class="coin-level-title" style="text-align: center; color: var(--primary-color); margin-bottom: 15px; border-bottom: 2px dashed #eee; padding-bottom: 10px;">${level}</h3>
                <div class="coin-package-list" style="display: flex; flex-direction: column; gap: 10px;">
                    ${pkgs.map(pkg => {
                        const isSelected = selectedCoinId === pkg.id;
                        const isDisabled = selectedCoinId && selectedCoinId !== pkg.id;
                        const iconUrl = pkg.icon || '';
                        const iconHtml = iconUrl ? `<img src="${iconUrl}" alt="coin" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover;">` : '';
                        ...
                        if (isSelected) {
                            ...
                            buttonHtml = `<button class="btn btn-small" disabled style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem; opacity: 0.4; cursor: not-allowed; background: #ccc; color: #888; border: none;">เลือก</button>`;
                        } else {
                        ...
                        return `
                        <div class="coin-package-item ${isSelected ? 'coin-pkg-selected' : ''} ${isDisabled ? 'coin-pkg-disabled' : ''}" style="display: flex; justify-content: space-between; align-items: center; background: ${isSelected ? 'linear-gradient(135deg, #f0fff4, #e6ffed)' : '#fff'}; padding: 10px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: ${isSelected ? '2px solid #27ae60' : '1px solid #f0f0f0'}; transition: all 0.3s ease; ${isDisabled ? 'opacity: 0.5;' : ''}; min-width: 0; overflow: hidden;">
                            <div class="coin-info" style="display: flex; align-items: center; gap: 8px; min-width: 0; overflow: hidden;">
                                ${iconHtml}
                                <span style="font-weight: bold; color: #ffa502; font-size: 1rem; min-width: 70px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.coins.toLocaleString()} 💰</span>
                                <span style="color: #666; font-size: 0.85rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.price.toLocaleString()} บาท</span>
                            </div>
                            ${buttonHtml}
                        </div>
                    `;
    ...
    // ===== START: VOUCHER FRONTEND FUNC =====
    window.addVoucherToCart = (id) => {
        const pkg = (appData.shopSettings.voucherPackages || []).find(p => p.id === id);
        if(!pkg) return;
        
        // ===== SINGLE-SELECT: Clear all previous voucher packages from cart =====
        const allVoucherIds = (appData.shopSettings.voucherPackages || []).map(p => p.id);
        allVoucherIds.forEach(vId => {
            if (appData.cart[vId]) delete appData.cart[vId];
        });
        ...
        localStorage.setItem('warishayday_cart', JSON.stringify(appData.cart));
        checkOrderValidation();
        renderVoucherCatalogFrontend(); // Re-render to show checkmark
        Notify.success('เลือกบัตรกำนัลแล้ว', `${pkg.quantity.toLocaleString()} ใบ ถูกเพิ่มลงบัญชีสั่งซื้อ`);
    };

    window.renderVoucherCatalogFrontend = () => {
        const productGrid = document.getElementById('product-grid');
        if (!productGrid) return;
        productGrid.innerHTML = '';
        
        const packages = appData.shopSettings.voucherPackages || [];
        if(packages.length === 0) {
            productGrid.innerHTML = `
                <div style="width: 100%; min-height: 400px; background-color: #ffffff; display: flex; align-items: center; justify-content: center; flex-direction: column; grid-column: 1 / -1; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <span style="font-size: 40px; color: #ccc; margin-bottom: 10px;">🎟️</span>
                    <span style="color: #888; font-family: var(--global-font-family); font-size: 1.2rem;">ยังไม่มีแพ็กเกจบัตรกำนัล...</span>
                </div>
            `;
            return;
        }

        const allVoucherIds = packages.map(p => p.id);
        const selectedVoucherId = allVoucherIds.find(vId => appData.cart[vId] && appData.cart[vId] > 0) || null;

        const container = document.createElement('div');
        container.className = 'voucher-catalog-container';
        container.style.cssText = 'width: 100%; grid-column: 1/-1; display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; padding-bottom: 20px;';

        packages.sort((a,b) => a.quantity - b.quantity);

        packages.forEach(pkg => {
            const isSelected = selectedVoucherId === pkg.id;
            const isDisabled = selectedVoucherId && selectedVoucherId !== pkg.id;
            const iconUrl = pkg.icon || '';
            const iconHtml = iconUrl ? `<img src="${iconUrl}" alt="voucher" style="width: 24px; height: 24px; border-radius: 4px; object-fit: cover; margin-right: 5px;">` : '';
            
            let btnOrCheck = '';
            if (isSelected) {
                btnOrCheck = `<span class="voucher-selected-check" style="display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; background: #27ae60; border-radius: 50%; color: #fff; font-size: 1rem; box-shadow: 0 2px 8px rgba(39,174,96,0.3); animation: coinCheckPop 0.3s ease;">✓</span>`;
            } else if (isDisabled) {
                btnOrCheck = `<button class="btn btn-small" disabled style="margin: 0; padding: 4px 8px; border-radius: 20px; font-size: 9px; opacity: 0.4; cursor: not-allowed; background: #ccc; color: #888; border: none; min-width: 45px;">เลือก</button>`;
            } else {
                btnOrCheck = `<button class="btn btn-primary btn-small" onclick="addVoucherToCart('${pkg.id}')" style="margin: 0; padding: 4px 8px; border-radius: 20px; font-size: 9px; min-width: 45px;">เลือก</button>`;
            }

            const itemDiv = document.createElement('div');
            itemDiv.className = `voucher-package-item ${isSelected ? 'selected' : ''} ${isDisabled ? 'disabled' : ''}`;
            itemDiv.style.cssText = `
                display: flex; align-items: center; justify-content: space-between; 
                background: ${isSelected ? '#e6ffed' : '#ffffff'}; 
                border: 2px solid ${isSelected ? '#27ae60' : '#f0f0f0'}; 
                border-radius: 10px; padding: 8px; transition: all 0.2s ease;
                box-shadow: 0 2px 5px rgba(0,0,0,0.05); ${isDisabled ? 'opacity: 0.5;' : ''}
                min-width: 0; overflow: hidden;
            `;

            itemDiv.innerHTML = `
                <div style="display: flex; align-items: center; flex: 1; gap: 5px; min-width: 0;">
                    ${iconHtml}
                    <div style="display: flex; flex-direction: column; text-align: left; min-width: 0; overflow: hidden;">
                        <span style="font-weight: bold; font-size: 9px; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.quantity.toLocaleString()} ใบ</span>
                        <span style="color: #e74c3c; font-weight: bold; font-size: 9px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.price.toLocaleString()} บาท</span>
                    </div>
                </div>
                ${btnOrCheck}
            `;
    ...
    // ===== START: DIAMOND & FARM PASS FUNC =====
    window.addDiamondToCart = (id) => {
        const pkg = (appData.shopSettings.diamondPackages || []).find(p => p.id === id);
        if(!pkg) return;
        
        // Single-select: Clear previous diamond packages
        const allPkgIds = (appData.shopSettings.diamondPackages || []).map(p => p.id);
        allPkgIds.forEach(pkgId => {
            if (appData.cart[pkgId]) delete appData.cart[pkgId];
        });
        ...
        checkOrderValidation();
        if (typeof renderDiamondPassCatalog === 'function') renderDiamondPassCatalog();
        Notify.success('เลือกเพชรแล้ว', `${pkg.diamonds.toLocaleString()} 💎 ถูกเพิ่มลงบัญชีสั่งซื้อ`);
    };

    window.addFarmPassToCart = (id) => {
        const pkg = (appData.shopSettings.farmPassPackages || []).find(p => p.id === id);
        if(!pkg) return;
        
        // Single-select: Clear previous farmpass packages
        const allPkgIds = (appData.shopSettings.farmPassPackages || []).map(p => p.id);
        allPkgIds.forEach(pkgId => {
            if (appData.cart[pkgId]) delete appData.cart[pkgId];
        });
        ...
        checkOrderValidation();
        if (typeof renderDiamondPassCatalog === 'function') renderDiamondPassCatalog();
        Notify.success('เลือก Farm Pass แล้ว', `${pkg.name} ถูกเพิ่มลงบัญชีสั่งซื้อ`);
    };

    window.renderDiamondPassCatalog = () => {
        const productGrid = document.getElementById('product-grid');
        if (!productGrid) return;
        productGrid.innerHTML = '';
        
        const diamonds = appData.shopSettings.diamondPackages || [];
        const farmPasses = appData.shopSettings.farmPassPackages || [];
        
        if(diamonds.length === 0 && farmPasses.length === 0) {
            productGrid.innerHTML = `
                <div style="width: 100%; min-height: 400px; background-color: #ffffff; display: flex; align-items: center; justify-content: center; flex-direction: column; grid-column: 1 / -1; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                    <span style="font-size: 40px; color: #ccc; margin-bottom: 10px;">💎</span>
                    <span style="color: #888; font-family: var(--global-font-family); font-size: 1.2rem;">ยังไม่มีแพ็กเกจเพชรและ Farm Pass...</span>
                </div>
            `;
            return;
        }

        const selectedDiamondId = diamonds.map(p => p.id).find(id => appData.cart[id] && appData.cart[id] > 0) || null;
        const selectedFarmPassId = farmPasses.map(p => p.id).find(id => appData.cart[id] && appData.cart[id] > 0) || null;

        const container = document.createElement('div');
        container.className = 'diamond-pass-catalog-container';
        container.style.cssText = 'width: 100%; grid-column: 1/-1; display: flex; flex-direction: column; gap: 30px; padding-bottom: 20px;';

        // 1. Diamonds Section
        if (diamonds.length > 0) {
            const diamondDiv = document.createElement('div');
            diamondDiv.className = 'diamond-section';
            
            // Note: sort by price or diamonds
            const sortedDiamonds = [...diamonds].sort((a,b) => a.diamonds - b.diamonds);
            
            diamondDiv.innerHTML = `
                <h3 style="text-align: center; color: #00d2ff; margin-bottom: 15px; border-bottom: 2px dashed #eee; padding-bottom: 10px;">💎 เติมเพชร</h3>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;">
                    ${sortedDiamonds.map(pkg => {
                        const isSelected = selectedDiamondId === pkg.id;
                        const isDisabled = selectedDiamondId && selectedDiamondId !== pkg.id;
                        const iconHtml = pkg.icon ? `<img src="${pkg.icon}" alt="diamond" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover;">` : '';
                        ...
                        let buttonHtml = isSelected 
                            ...
                            : (isDisabled 
                                ? `<button class="btn btn-small" disabled style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem; opacity: 0.4; cursor: not-allowed; background: #ccc; color: #888; border: none;">เลือก</button>`
                        ...
                        return `
                        <div class="diamond-pkg-item" style="display: flex; justify-content: space-between; align-items: center; background: ${isSelected ? 'linear-gradient(135deg, #f0fff4, #e6ffed)' : '#fff'}; padding: 10px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: ${isSelected ? '2px solid #27ae60' : '1px solid #e0f7fa'}; transition: all 0.3s ease; ${isDisabled ? 'opacity: 0.6;' : ''}; min-width: 0; overflow: hidden;">
                            <div style="display: flex; align-items: center; gap: 8px; min-width: 0; overflow: hidden;">
                                ${iconHtml}
                                <span style="font-weight: bold; color: #0093e9; font-size: 1rem; min-width: 70px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.diamonds.toLocaleString()} 💎</span>
                                <span style="color: #666; font-size: 0.85rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.price.toLocaleString()} บาท</span>
                            </div>
                            ${buttonHtml}
                        </div>`;
            ...
            `;
            container.appendChild(diamondDiv);
        }

        // 2. Farm Pass Section
        if (farmPasses.length > 0) {
            const passDiv = document.createElement('div');
            passDiv.className = 'farmpass-section';
            
            const sortedPasses = [...farmPasses].sort((a,b) => a.price - b.price);
            
            passDiv.innerHTML = `
                <h3 style="text-align: center; color: #4facfe; margin-bottom: 15px; border-bottom: 2px dashed #eee; padding-bottom: 10px; margin-top: ${diamonds.length > 0 ? '10px' : '0'};">🎫 Farm Pass</h3>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px;">
                    ${sortedPasses.map(pkg => {
                        const isSelected = selectedFarmPassId === pkg.id;
                        const isDisabled = selectedFarmPassId && selectedFarmPassId !== pkg.id;
                        const imgHtml = pkg.image ? `<img src="${pkg.image}" alt="farm pass" style="width: 100%; aspect-ratio: 1/1; object-fit: cover; border-radius: 12px; margin-bottom: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">` : `<div style="width: 100%; aspect-ratio: 1/1; background: #f0f0f0; border-radius: 12px; margin-bottom: 15px; display:flex; align-items:center; justify-content:center; color:#ccc; font-size: 3rem;">🎫</div>`;
                        
                        let buttonHtml = isSelected 
                            ? `<span class="coin-selected-check" style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; background: #27ae60; border-radius: 50%; color: #fff; font-size: 1.2rem; box-shadow: 0 2px 8px rgba(39,174,96,0.3); animation: coinCheckPop 0.3s ease;">✓ (เลือกแล้ว)</span>`
                            : (isDisabled 
                                ? `<button class="btn btn-block" disabled style="padding: 10px; border-radius: 8px; font-size: 1rem; opacity: 0.4; cursor: not-allowed; background: #ccc; color: #555; border: none; font-weight: bold;">เลือก</button>`
                        ...
                        return `
                        <div class="farmpass-pkg-item" style="display: flex; flex-direction: column; background: ${isSelected ? '#eaffef' : '#fff'}; padding: 15px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.08); border: ${isSelected ? '2px solid #27ae60' : '1px solid #f0f0f0'}; transition: all 0.3s ease; align-items: center; text-align: center; ${isDisabled ? 'opacity: 0.6;' : ''}">
                            <div style="width: 60%; max-width: 180px;">${imgHtml}</div>
                            <h4 style="margin: 0 0 10px 0; color: #333; font-size: 1.1rem;">${pkg.name}</h4>
                            <div style="font-weight: bold; color: #ff5722; font-size: 1.2rem; margin-bottom: 15px;">${pkg.price.toLocaleString()} บาท</div>
    ...
    document.getElementById('proceed-confirm-order-btn').addEventListener('click', () => {
    ...
    const renderAdminPanel = async () => {
        ...
        if (activeAdminMenu === 'admin' && canAccess('admin')) {
            ...
            } else if (activeSub === 'coin-packages') {
                renderAdminCoinPackages();
            } else if (activeSub === 'diamond-pass') {
            ...
            } else if (activeSub === 'voucher-packages') {
                if (typeof renderAdminVoucherPackages === 'function') renderAdminVoucherPackages();
    ...
    const renderDashboard = () => {
        ...
        // ===== Section 1.5: Virtual Package Dashboard (Real-time) =====
        if (window.DashboardModule && window.DashboardModule.renderVirtualPackageDashboard) {
            window.DashboardModule.renderVirtualPackageDashboard();
        }
    };
    ...
    const populateProductSelect = () => {
        ...
        const virtualGroups = [
            { label: '💰 เหรียญ',     items: appData.shopSettings?.coinPackages    || [], getLabel: p => `${(p.coins||'').toLocaleString()} เหรียญ (${(p.price||0).toLocaleString()} ฿)` },
            { label: '💎 เพชร',       items: appData.shopSettings?.diamondPackages || [], getLabel: p => `${(p.diamonds||'').toLocaleString()} เพชร (${(p.price||0).toLocaleString()} ฿)` },
            { label: '🎫 Farm Pass',  items: appData.shopSettings?.farmpassPackages|| [], getLabel: p => p.name || p.id },
            { label: '🎟️ บัตรกำนัล', items: appData.shopSettings?.voucherPackages || [], getLabel: p => `${(p.quantity||'').toLocaleString()} ใบ (${(p.price||0).toLocaleString()} ฿)` },
        ];

        virtualGroups.forEach(({ label, items, getLabel }) => {
    ...
    // ===== START: COIN PACKAGES LOGIC =====
    let editingCoinPkgId = null;

    document.getElementById('coin-package-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const submitBtn = e.target.querySelector('button[type="submit"]');
        showSaveFeedback(submitBtn);

        const levelRange = document.getElementById('coin-level-range').value.trim();
        const coins = parseInt(document.getElementById('coin-amount').value, 10);
        const price = parseInt(document.getElementById('coin-price').value, 10);
        const icon = document.getElementById('coin-icon-url').value.trim();

        if (!levelRange || isNaN(coins) || isNaN(price)) {
            Notify.warning('กรุณากรอกข้อมูล', 'กรุณากรอกข้อมูลให้ครบถ้วนและถูกต้อง');
            submitBtn.disabled = false;
            submitBtn.textContent = 'เพิ่มแพ็กเกจเหรียญ';
            return;
        }

        if (!appData.shopSettings.coinPackages) appData.shopSettings.coinPackages = [];

        if (editingCoinPkgId) {
            const index = appData.shopSettings.coinPackages.findIndex(p => p.id === editingCoinPkgId);
            if (index !== -1) {
                appData.shopSettings.coinPackages[index] = { id: editingCoinPkgId, levelRange, coins, price, icon };
                addLog('Coin Package Updated', `Range: ${levelRange}, Coins: ${coins}, Price: ${price}`);
            }
            editingCoinPkgId = null;
        } else {
            const newPkg = { id: 'coin_' + Date.now(), levelRange, coins, price, icon };
            appData.shopSettings.coinPackages.push(newPkg);
            addLog('Coin Package Created', `Range: ${levelRange}, Coins: ${coins}, Price: ${price}`);
        }

        document.getElementById('coin-package-form').reset();
        document.getElementById('coin-icon-preview').style.backgroundImage = 'none';
        document.getElementById('coin-icon-preview').textContent = '💰';
        document.getElementById('submit-coin-btn').textContent = 'เพิ่มแพ็กเกจเหรียญ';
        document.getElementById('cancel-coin-edit-btn').style.display = 'none';

        await saveState();
        if (typeof renderAdminCoinPackages === 'function') renderAdminCoinPackages();
        
        Notify.success('สำเร็จ', 'บันทึกข้อมูลสำเร็จ');
        submitBtn.disabled = false;
        submitBtn.textContent = 'เพิ่มแพ็กเกจเหรียญ';
    });

    document.getElementById('cancel-coin-edit-btn').addEventListener('click', () => {
        editingCoinPkgId = null;
        document.getElementById('coin-package-form').reset();
        document.getElementById('coin-icon-preview').style.backgroundImage = 'none';
        document.getElementById('coin-icon-preview').textContent = '💰';
    ...
    window.editCoinPackage = (id) => {
        const pkg = appData.shopSettings.coinPackages.find(p => p.id === id);
        if (pkg) {
            editingCoinPkgId = id;
            document.getElementById('coin-level-range').value = pkg.levelRange;
            document.getElementById('coin-amount').value = pkg.coins;
            document.getElementById('coin-price').value = pkg.price;
            document.getElementById('coin-icon-url').value = pkg.icon || '';
            const preview = document.getElementById('coin-icon-preview');
            if (pkg.icon) {
                preview.style.backgroundImage = `url(${pkg.icon})`;
                preview.textContent = '';
            } else {
                preview.style.backgroundImage = 'none';
                preview.textContent = '💰';
            }
            document.getElementById('submit-coin-btn').textContent = 'บันทึกการแก้ไข';
            document.getElementById('cancel-coin-edit-btn').style.display = 'inline-block';
            document.getElementById('coin-package-form').scrollIntoView({ behavior: 'smooth' });
        }
    };
    ...
    window.renderAdminCoinPackages = () => {
        const tbody = document.getElementById('admin-coin-package-list');
        if (!tbody) return;
        tbody.innerHTML = '';
        
        if (!appData.shopSettings.coinPackages || appData.shopSettings.coinPackages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align: center; color: var(--text-color);">ไม่มีข้อมูลแพ็กเกจ</td></tr>';
            return;
        }

        appData.shopSettings.coinPackages.forEach(pkg => {
            const tr = document.createElement('tr');
            const iconPreview = pkg.icon 
                ? `<img src="${pkg.icon}" alt="icon" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover;">` 
                : `<span style="color: #ccc;">-</span>`;
            tr.innerHTML = `
                <td style="text-align: center;">${iconPreview}</td>
                <td style="text-align: left;">${pkg.levelRange}</td>
                <td style="text-align: right; color: var(--primary-color); font-weight: bold;">${pkg.coins.toLocaleString()} 💰</td>
                <td style="text-align: right;">${pkg.price.toLocaleString()} บาท</td>
                <td style="text-align: center;">
                    <div style="display: flex; gap: 5px; justify-content: center;">
                        <button class="btn btn-secondary btn-small" onclick="editCoinPackage('${pkg.id}')">แก้ไข</button>
                        <button class="btn btn-danger btn-small" onclick="deleteCoinPackage('${pkg.id}')">ลบ</button>
                    </div>
                </td>
            `;
    ...
    // ===== START: ADMIN VOUCHER CRUD =====
    let editingVoucherPkgId = null;

    document.getElementById('voucher-icon-url').addEventListener('input', (e) => {
        const preview = document.getElementById('voucher-icon-preview');
        if (e.target.value.trim()) {
            preview.style.backgroundImage = `url(${e.target.value.trim()})`;
            preview.textContent = '';
        } else {
            preview.style.backgroundImage = 'none';
    ...
    document.getElementById('coin-package-form').insertAdjacentHTML('afterend', `
        <!-- Form is added dynamically below to preserve ID space safely, wait it's better to add the form in HTML, but since I can't easily jump to index.html without a context switch, I will add it via JS if possible. Actually wait, I have the index.html ready to edit. -->
    `);

    document.getElementById('submit-voucher-btn')?.addEventListener('click', async (e) => {
        e.preventDefault();
        const quantityStr = document.getElementById('voucher-amount').value;
        const priceStr = document.getElementById('voucher-price').value;
        const icon = document.getElementById('voucher-icon-url').value;

        if (!quantityStr || !priceStr) {
            Notify.warning('ข้อมูลไม่ครบ', 'กรุณากรอกจำนวนใบและราคา');
            return;
        }
        ...
        if (isNaN(quantity) || quantity <= 0 || isNaN(price) || price < 0) {
            Notify.warning('ข้อมูลไม่ถูกต้อง', 'รูปแบบข้อมูลไม่ถูกต้อง');
            return;
        }
        ...
        if (!appData.shopSettings.voucherPackages) appData.shopSettings.voucherPackages = [];

        if (editingVoucherPkgId) {
            const pkg = appData.shopSettings.voucherPackages.find(p => p.id === editingVoucherPkgId);
            if (pkg) {
                pkg.quantity = quantity;
                pkg.price = price;
                pkg.icon = icon;
                addLog('Voucher Package Updated', `Quantity: ${quantity}, Price: ${price}`);
            }
            editingVoucherPkgId = null;
        } else {
            const newPkg = { id: 'voucher_' + Date.now(), quantity, price, icon };
            appData.shopSettings.voucherPackages.push(newPkg);
            addLog('Voucher Package Created', `Quantity: ${quantity}, Price: ${price}`);
        }

        document.getElementById('voucher-package-form').reset();
        document.getElementById('voucher-icon-preview').style.backgroundImage = 'none';
        document.getElementById('voucher-icon-preview').textContent = '🎟️';
        document.getElementById('submit-voucher-btn').textContent = 'เพิ่มบัตรกำนัล';
        document.getElementById('cancel-voucher-edit-btn').style.display = 'none';
        ...
        Notify.success('สำเร็จ', 'บันทึกข้อมูลสำเร็จ');
        submitBtn.disabled = false;
        submitBtn.textContent = 'เพิ่มบัตรกำนัล';
    });

    document.getElementById('cancel-voucher-edit-btn')?.addEventListener('click', () => {
        editingVoucherPkgId = null;
        document.getElementById('voucher-package-form').reset();
        document.getElementById('voucher-icon-preview').style.backgroundImage = 'none';
        document.getElementById('voucher-icon-preview').textContent = '🎟️';
    ...
    window.editVoucherPackage = (id) => {
        const pkg = (appData.shopSettings.voucherPackages || []).find(p => p.id === id);
        if (pkg) {
            editingVoucherPkgId = id;
            document.getElementById('voucher-amount').value = pkg.quantity;
            document.getElementById('voucher-price').value = pkg.price;
            document.getElementById('voucher-icon-url').value = pkg.icon || '';
            const preview = document.getElementById('voucher-icon-preview');
            if (pkg.icon) {
                preview.style.backgroundImage = `url(${pkg.icon})`;
                preview.textContent = '';
            ...
            document.getElementById('submit-voucher-btn').textContent = 'บันทึกการแก้ไข';
            document.getElementById('cancel-voucher-edit-btn').style.display = 'inline-block';
            document.getElementById('voucher-package-form').scrollIntoView({ behavior: 'smooth' });
        }
    };
    ...
    window.renderAdminVoucherPackages = () => {
        const tbody = document.getElementById('admin-voucher-package-list');
        if (!tbody) return;
        tbody.innerHTML = '';
        
        if (!appData.shopSettings.voucherPackages || appData.shopSettings.voucherPackages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-color);">ไม่มีข้อมูลบัตรกำนัล</td></tr>';
            return;
        }

        appData.shopSettings.voucherPackages.sort((a,b) => a.quantity - b.quantity).forEach(pkg => {
            const tr = document.createElement('tr');
            const iconPreview = pkg.icon 
                ? `<img src="${pkg.icon}" alt="icon" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover;">` 
                : `<span style="color: #ccc;">-</span>`;
            tr.innerHTML = `
                <td style="text-align: center;">${iconPreview}</td>
                <td style="text-align: right; color: var(--primary-color); font-weight: bold;">${pkg.quantity.toLocaleString()} ใบ</td>
                <td style="text-align: right;">${pkg.price.toLocaleString()} บาท</td>
                <td style="text-align: center;">
                    <div style="display: flex; gap: 5px; justify-content: center;">
                        <button class="btn btn-secondary btn-small" onclick="editVoucherPackage('${pkg.id}')">แก้ไข</button>
                        <button class="btn btn-danger btn-small" onclick="deleteVoucherPackage('${pkg.id}')">ลบ</button>
                    </div>
                </td>
            `;
            tbody.appendChild(tr);
    ...
    // ===== START: ADMIN DIAMOND & FARM PASS CRUD =====
    let editingDiamondPkgId = null;
    let editingFarmPassPkgId = null;

    // Diamond Form Handlers
    document.getElementById('diamond-icon-url').addEventListener('input', (e) => {
        const preview = document.getElementById('diamond-icon-preview');
        if (e.target.value.trim()) {
            preview.style.backgroundImage = `url(${e.target.value.trim()})`;
            preview.textContent = '';
        } else {
    ...
    document.getElementById('diamond-package-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const submitBtn = document.getElementById('submit-diamond-btn');
        submitBtn.disabled = true;
        submitBtn.textContent = 'กำลังบันทึก...';

        const diamonds = parseInt(document.getElementById('diamond-amount').value);
        const price = parseFloat(document.getElementById('diamond-price').value);
        const icon = document.getElementById('diamond-icon-url').value.trim();

        if (isNaN(diamonds) || isNaN(price)) {
            Notify.error('Error', 'กรุณากรอกข้อมูลตัวเลขให้ถูกต้อง');
            submitBtn.disabled = false;
            submitBtn.textContent = 'เพิ่มแพ็กเกจเพชร';
            return;
        }

        if (!appData.shopSettings.diamondPackages) appData.shopSettings.diamondPackages = [];

        if (editingDiamondPkgId) {
            const index = appData.shopSettings.diamondPackages.findIndex(p => p.id === editingDiamondPkgId);
            if (index !== -1) {
                appData.shopSettings.diamondPackages[index] = { id: editingDiamondPkgId, diamonds, price, icon };
            }
            editingDiamondPkgId = null;
        } else {
            const newPkg = { id: 'diamond_' + Date.now(), diamonds, price, icon };
            appData.shopSettings.diamondPackages.push(newPkg);
        }

        document.getElementById('diamond-package-form').reset();
        document.getElementById('diamond-icon-preview').style.backgroundImage = 'none';
        document.getElementById('diamond-icon-preview').textContent = '💎';
        document.getElementById('submit-diamond-btn').textContent = 'เพิ่มแพ็กเกจเพชร';
        document.getElementById('cancel-diamond-edit-btn').style.display = 'none';

        await saveState();
        if (typeof renderAdminDiamondPackages === 'function') renderAdminDiamondPackages();
        
        Notify.success('สำเร็จ', 'บันทึกข้อมูลเพชรสำเร็จ');
        submitBtn.disabled = false;
    });

    document.getElementById('cancel-diamond-edit-btn').addEventListener('click', () => {
        editingDiamondPkgId = null;
        document.getElementById('diamond-package-form').reset();
        document.getElementById('diamond-icon-preview').style.backgroundImage = 'none';
        document.getElementById('diamond-icon-preview').textContent = '💎';
    ...
    window.editDiamondPackage = (id) => {
        const pkg = appData.shopSettings.diamondPackages.find(p => p.id === id);
        if (pkg) {
            editingDiamondPkgId = id;
            document.getElementById('diamond-amount').value = pkg.diamonds;
            document.getElementById('diamond-price').value = pkg.price;
            document.getElementById('diamond-icon-url').value = pkg.icon || '';
            const preview = document.getElementById('diamond-icon-preview');
            if (pkg.icon) {
                preview.style.backgroundImage = `url(${pkg.icon})`;
                preview.textContent = '';
            ...
            document.getElementById('submit-diamond-btn').textContent = 'บันทึกการแก้ไข';
            document.getElementById('cancel-diamond-edit-btn').style.display = 'inline-block';
            document.getElementById('diamond-package-form').scrollIntoView({ behavior: 'smooth' });
        }
    };
    ...
    window.renderAdminDiamondPackages = () => {
        const tbody = document.getElementById('admin-diamond-package-list');
        if (!tbody) return;
        tbody.innerHTML = '';
        
        if (!appData.shopSettings.diamondPackages || appData.shopSettings.diamondPackages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-color);">ไม่มีข้อมูลแพ็กเกจเพชร</td></tr>';
            return;
        }

        appData.shopSettings.diamondPackages.forEach(pkg => {
            const tr = document.createElement('tr');
            const iconPreview = pkg.icon 
                ? `<img src="${pkg.icon}" alt="icon" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover;">` 
                : `<span style="color: #ccc;">-</span>`;
            tr.innerHTML = `
                <td style="text-align: center;">${iconPreview}</td>
                <td style="text-align: right; color: #0093e9; font-weight: bold;">${pkg.diamonds.toLocaleString()} 💎</td>
                <td style="text-align: right;">${pkg.price.toLocaleString()} บาท</td>
                <td style="text-align: center;">
                    <div style="display: flex; gap: 5px; justify-content: center;">
                        <button class="btn btn-secondary btn-small" onclick="editDiamondPackage('${pkg.id}')">แก้ไข</button>
                        <button class="btn btn-danger btn-small" onclick="deleteDiamondPackage('${pkg.id}')">ลบ</button>
                    </div>
                </td>
            `;
            tbody.appendChild(tr);
    ...
    // Farm Pass Form Handlers
    document.getElementById('farmpass-image-url').addEventListener('input', (e) => {
        const preview = document.getElementById('farmpass-icon-preview');
        if (e.target.value.trim()) {
            preview.style.backgroundImage = `url(${e.target.value.trim()})`;
            preview.textContent = '';
        } else {
    ...
    document.getElementById('farmpass-package-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const submitBtn = document.getElementById('submit-farmpass-btn');
        submitBtn.disabled = true;
        submitBtn.textContent = 'กำลังบันทึก...';

        const name = document.getElementById('farmpass-name').value.trim();
        const price = parseFloat(document.getElementById('farmpass-price').value);
        const image = document.getElementById('farmpass-image-url').value.trim();

        if (!name || isNaN(price)) {
            Notify.error('Error', 'กรุณากรอกข้อมูลให้ครบถ้วน');
            submitBtn.disabled = false;
            submitBtn.textContent = 'เพิ่ม Farm Pass';
            return;
        }

        if (!appData.shopSettings.farmPassPackages) appData.shopSettings.farmPassPackages = [];

        if (editingFarmPassPkgId) {
            const index = appData.shopSettings.farmPassPackages.findIndex(p => p.id === editingFarmPassPkgId);
            if (index !== -1) {
                appData.shopSettings.farmPassPackages[index] = { id: editingFarmPassPkgId, name, price, image };
            }
            editingFarmPassPkgId = null;
        } else {
            const newPkg = { id: 'farmpass_' + Date.now(), name, price, image };
            appData.shopSettings.farmPassPackages.push(newPkg);
        }

        document.getElementById('farmpass-package-form').reset();
        document.getElementById('farmpass-icon-preview').style.backgroundImage = 'none';
        document.getElementById('farmpass-icon-preview').textContent = '🎫';
        document.getElementById('submit-farmpass-btn').textContent = 'เพิ่ม Farm Pass';
        document.getElementById('cancel-farmpass-edit-btn').style.display = 'none';

        await saveState();
        if (typeof renderAdminFarmPassPackages === 'function') renderAdminFarmPassPackages();
        
        Notify.success('สำเร็จ', 'บันทึกข้อมูล Farm Pass สำเร็จ');
        submitBtn.disabled = false;
    });

    document.getElementById('cancel-farmpass-edit-btn').addEventListener('click', () => {
        editingFarmPassPkgId = null;
        document.getElementById('farmpass-package-form').reset();
        document.getElementById('farmpass-icon-preview').style.backgroundImage = 'none';
        document.getElementById('farmpass-icon-preview').textContent = '🎫';
    ...
    window.editFarmPassPackage = (id) => {
        const pkg = appData.shopSettings.farmPassPackages.find(p => p.id === id);
        if (pkg) {
            editingFarmPassPkgId = id;
            document.getElementById('farmpass-name').value = pkg.name;
            document.getElementById('farmpass-price').value = pkg.price;
            document.getElementById('farmpass-image-url').value = pkg.image || '';
            const preview = document.getElementById('farmpass-icon-preview');
            if (pkg.image) {
                preview.style.backgroundImage = `url(${pkg.image})`;
                preview.textContent = '';
            } else {
                preview.style.backgroundImage = 'none';
                preview.textContent = '🎫';
            }
            document.getElementById('submit-farmpass-btn').textContent = 'บันทึกการแก้ไข';
            document.getElementById('cancel-farmpass-edit-btn').style.display = 'inline-block';
            document.getElementById('farmpass-package-form').scrollIntoView({ behavior: 'smooth' });
        }
    };
    ...
    window.renderAdminFarmPassPackages = () => {
        const tbody = document.getElementById('admin-farmpass-package-list');
        if (!tbody) return;
        tbody.innerHTML = '';
        
        if (!appData.shopSettings.farmPassPackages || appData.shopSettings.farmPassPackages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-color);">ไม่มีข้อมูล Farm Pass</td></tr>';
            return;
        }

        appData.shopSettings.farmPassPackages.forEach(pkg => {
            const tr = document.createElement('tr');
            const iconPreview = pkg.image 
                ? `<img src="${pkg.image}" alt="poster" style="width: 40px; height: 40px; border-radius: 6px; object-fit: cover;">` 
                : `<span style="color: #ccc;">-</span>`;
            tr.innerHTML = `
                <td style="text-align: center;">${iconPreview}</td>
                <td style="text-align: left; font-weight: bold;">${pkg.name}</td>
                <td style="text-align: right;">${pkg.price.toLocaleString()} บาท</td>
                <td style="text-align: center;">
                    <div style="display: flex; gap: 5px; justify-content: center;">
                        <button class="btn btn-secondary btn-small" onclick="editFarmPassPackage('${pkg.id}')">แก้ไข</button>
                        <button class="btn btn-danger btn-small" onclick="deleteFarmPassPackage('${pkg.id}')">ลบ</button>
                    </div>
                </td>
            `;
            tbody.appendChild(tr);
    ...
    // ===== END: ADMIN DIAMOND & FARM PASS CRUD =====
    
    document.getElementById('toggle-coin-form-btn').addEventListener('click', (e) => {
        const form = document.getElementById('coin-package-form');
        const isHidden = form.style.display === 'none';
        form.style.display = isHidden ? 'block' : 'none';
        e.target.textContent = isHidden ? 'ซ่อน' : 'แสดง';
    });

    document.getElementById('toggle-diamond-form-btn').addEventListener('click', (e) => {
        const form = document.getElementById('diamond-package-form');
        const isHidden = form.style.display === 'none';
        form.style.display = isHidden ? 'block' : 'none';
        e.target.textContent = isHidden ? 'ซ่อน' : 'แสดง';
    });

    document.getElementById('toggle-farmpass-form-btn').addEventListener('click', (e) => {
        const form = document.getElementById('farmpass-package-form');
        const isHidden = form.style.display === 'none';
        form.style.display = isHidden ? 'block' : 'none';
        e.target.textContent = isHidden ? 'ซ่อน' : 'แสดง';
    });

    // ===== Live preview for coin icon URL =====
    document.getElementById('coin-icon-url').addEventListener('input', (e) => {
        const preview = document.getElementById('coin-icon-preview');
    ...
    const viewOrderDetails = (orderId) => {
        ...
        // ===== ADMIN VIEW: Use visual HTML (same format as confirm order) =====
        orderDetails.innerHTML = createConfirmOrderHTML(displayOrderId);

        appData.cart = originalCart;
        currentAppliedPromo = originalPromo;

        // ===== Set title with order number =====
        document.getElementById('order-modal-title').textContent = 'รายละเอียดออเดอร์';
        
        // ===== Show order number badge =====
        const badge = document.getElementById('order-number-badge');
        if (badge) {
            badge.innerHTML = `<span class="badge-label">เลขที่ออเดอร์:</span> <span class="badge-number">${displayOrderId}</span>`;
            badge.style.display = 'flex';
        }

        // ===== VIEW-ONLY: Hide all action buttons except Close =====
        ...
        document.getElementById('close-order-modal-btn').style.display = 'block';
    ...
    const renderDashboardCategoryTable = () => {
        ...
        // Virtual category display names
        const virtualCatNames = {
            'coin_purchases':     '💰 เหรียญ',
            'diamond_purchases':  '💎 เพชร',
            'farmpass_purchases': '🎫 Farm Pass',
            'voucher_purchases':  '🎟️ บัตรกำนัล'
        };
        const virtualCatIds = Object.keys(virtualCatNames);
        ...
        confirmedOrders.forEach(order => {
            ...
            itemsList.forEach(item => {
                ...
                // Try virtual product first, then normal product
                let product = null;
                if (typeof window.getProductById === 'function') {
                    product = window.getProductById(prodId);
                }
                if (!product) product = appData.allProducts.find(p => p.id == prodId);
                ...
                if (product) {
                    ...
                    let catName = null;
                    if (virtualCatIds.includes(catId)) {
                        catName = virtualCatNames[catId];
                    } else {
                        const cat = appData.categories.find(c => c.id == catId);
                    ...
                    if (catName) {
                        if (!salesByCategory[catId]) {
                            salesByCategory[catId] = { name: catName, quantity: 0, revenue: 0 };
                        }
                        salesByCategory[catId].quantity += qty;

                        // Virtual packages: price × qty directly; normal: calculatePrice
                        let revenue = 0;
                        if (virtualCatIds.includes(catId)) {
                            revenue = parseFloat(product.price || 0) * qty;
                        } else if (typeof calculatePrice === 'function') {
                            revenue = calculatePrice(parseInt(catId), qty).price;
                        }
                        salesByCategory[catId].revenue += revenue;
                        totalSales += revenue;
                    }
                }
            });
        ...
        if (totalCountEl) totalCountEl.textContent = sortedCategories.length;
    ...
    const init = async () => {
        ...
        // ===== FIX: โหลดข้อมูลร้านค้าให้เสร็จก่อน render =====
        // เช็คว่ามี ?store= ใน URL หรือไม่ ถ้ามีให้ await checkUrlForStore() ก่อน render
        // เพื่อให้ข้อมูล สินค้า / เหรียญ / เพชร / บัตรกำนัล พร้อมแสดงทันทีพร้อมกัน
        const _initUrlParams = new URLSearchParams(window.location.search);
        if (_initUrlParams.get('store')) {
            try {
                await checkUrlForStore();
            } catch (e) {
                console.warn('checkUrlForStore during init failed:', e);
            }
        }
        // ===== END FIX =====

        // ===== INITIALIZE DEFAULT VOUCHER PACKAGES IF EMPTY =====
        if (!appData.shopSettings.voucherPackages || appData.shopSettings.voucherPackages.length === 0) {
            appData.shopSettings.voucherPackages = [
                { id: 'voucher_01', quantity: 50, price: 49, icon: '' },
                { id: 'voucher_02', quantity: 100, price: 99, icon: '' },
                { id: 'voucher_03', quantity: 200, price: 189, icon: '' },
                { id: 'voucher_04', quantity: 300, price: 259, icon: '' },
                { id: 'voucher_05', quantity: 500, price: 359, icon: '' },
                { id: 'voucher_06', quantity: 1000, price: 459, icon: '' },
                { id: 'voucher_07', quantity: 2000, price: 859, icon: '' },
                { id: 'voucher_08', quantity: 3000, price: 1399, icon: '' },
                { id: 'voucher_09', quantity: 5000, price: 1899, icon: '' },
                { id: 'voucher_10', quantity: 10000, price: 3499, icon: '' }
            ];
            // Don't save immediately to avoid unwanted writes on guest visit, 
    ...
    const ManagerStore = (() => {
        ...
        function approveRegistration(id, packageType) {
            ...
            // แสดง Loading หรือปิดปุ่มเพื่อป้องกันการกดซ้ำ (เลือกทำในขั้นต่อไปถ้าจำเป็น)
            ...
            .then(data => {
                ...
                // ค่อยย้ายข้อมูลในเครื่องเมื่อชัวร์ว่าเซิร์ฟเวอร์บันทึกแล้ว
                const confirmedIndex = storeRegistrations.findIndex(s => s.id === id);
                if (confirmedIndex !== -1) {
                    const confirmedStore = storeRegistrations[confirmedIndex];
                    confirmedStore.status = 'approved';
                    confirmedStore.packageType = packageType;
                    confirmedStore.approvedAt = new Date().toISOString();

                    pendingStores.push({ ...confirmedStore, serialKey: null });
                    storeRegistrations.splice(confirmedIndex, 1);
                }
            ...
            .catch(err => {
                ...
                Notify.error('เกิดข้อผิดพลาด', err.message || 'ไม่สามารถบันทึกการอนุมัติได้');
            });
        }
        ...
        function rejectRegistration(id) {
            ...
            .then(data => {
                ...
                if (confirmedIndex !== -1) {
                    const confirmedStore = storeRegistrations[confirmedIndex];
                    confirmedStore.status = 'rejected';
                    confirmedStore.rejectedAt = new Date().toISOString();
                    storeRegistrations.splice(confirmedIndex, 1);
                }
            ...
            .catch(err => {
                ...
                Notify.error('เกิดข้อผิดพลาด', err.message || 'ไม่สามารถบันทึกข้อมูลได้');
            });
        }
        ...
        function renderStoreRegistrations() {
            ...
            // ---- PENDING TAB ----
            const tbody = document.getElementById('store-registrations-list');
            if (tbody) {
                ...
                tbody.innerHTML = pendingList.map(store => {
                    ...
                    const ownerName = store.ownerName || store.username || '-';
                    const registeredAt = store.registeredAt || store.registered_at;
                    const contact = [
                        store.line ? `LINE: ${store.line}` : null,
                        store.facebook ? `FB: ${store.facebook}` : null,
                    ...
                    <tr>
                        ...
                        <td><span class="ms-status-badge pending">⏳ รอดำเนินการ</span></td>
                        <td>
                            <div class="action-btn-group">
                                <button class="ms-action-btn info" onclick="ManagerStore.showRegistrationDetails(${store.id})">👁️ ดู</button>
                                <button class="ms-action-btn success" onclick="ManagerStore.confirmApproveRegistration(${store.id})">✓ อนุมัติ</button>
                                <button class="ms-action-btn danger" onclick="ManagerStore.confirmRejectRegistration(${store.id})">✗ ไม่อนุมัติ</button>
                            </div>
                        </td>
                    </tr>`;
                }).join('') || '<tr><td colspan="6" class="ms-empty-state">📭 ไม่มีร้านค้ารอดำเนินการ</td></tr>';
            ...
            // ---- APPROVED TAB ----
            const approvedTbody = document.getElementById('store-approved-list');
            if (approvedTbody) {
                const allApproved = [...activeStores, ...pendingStores].sort((a, b) => new Date(b.registeredAt || 0) - new Date(a.registeredAt || 0));
                approvedTbody.innerHTML = allApproved.map(store => {
                    const contact = [
                        store.line ? `LINE: ${store.line}` : null,
                        store.facebook ? `FB: ${store.facebook}` : null,
                        store.phone ? `📞 ${store.phone}` : null,
                    ].filter(Boolean).join('<br>') || '-';
                    const approvedAt = store.approvedAt || store.approved_at || store.registeredAt;
                    return `
                    <tr>
                        <td>${store.shopName || '-'}</td>
                        <td>${store.ownerName || store.username || '-'}</td>
                        <td style="font-size:0.82rem;">${contact}</td>
                        <td>${store.registeredAt ? new Date(store.registeredAt).toLocaleDateString('th-TH') : '-'}</td>
                        <td>${approvedAt ? new Date(approvedAt).toLocaleDateString('th-TH') : '-'}</td>
                        <td>
                            <button class="ms-action-btn info" onclick="ManagerStore.showRegistrationDetails(${store.id})">👁️ ดู</button>
                        </td>
                    </tr>`;
                }).join('') || '<tr><td colspan="6" class="ms-empty-state">📭 ยังไม่มีร้านค้าที่อนุมัติ</td></tr>';

                const approvedTodayEl = document.getElementById('ms-approved-today');
                if (approvedTodayEl) {
                    const today = new Date().toDateString();
                    approvedTodayEl.textContent = allApproved.filter(s => {
                        const d = s.approvedAt || s.approved_at || s.registeredAt;
            ...
            // ---- REJECTED TAB ----
            const rejectedTbody = document.getElementById('store-rejected-list');
            if (rejectedTbody) {
                const rejectedList = storeRegistrations.filter(s => ['rejected', 'cancelled'].includes(s.status));
                rejectedTbody.innerHTML = rejectedList.map(store => {
                    const contact = [
                        store.line ? `LINE: ${store.line}` : null,
                        store.facebook ? `FB: ${store.facebook}` : null,
                        store.phone ? `📞 ${store.phone}` : null,
                    ].filter(Boolean).join('<br>') || '-';
                    const rejectedAt = store.rejectedAt || store.rejected_at || store.updatedAt || store.registeredAt;
                    return `
                    <tr>
                        ...
                        <td style="font-size:0.82rem;">${contact}</td>
                        <td>${store.registeredAt ? new Date(store.registeredAt).toLocaleDateString('th-TH') : '-'}</td>
                        <td>${rejectedAt ? new Date(rejectedAt).toLocaleDateString('th-TH') : '-'}</td>
                        <td>
                            <button class="ms-action-btn info" onclick="ManagerStore.showRegistrationDetails(${store.id})">👁️ ดู</button>
                        </td>
                    </tr>`;
                }).join('') || '<tr><td colspan="6" class="ms-empty-state">📭 ยังไม่มีร้านค้าที่ไม่อนุมัติ</td></tr>';

                const rejectedTodayEl = document.getElementById('ms-rejected-today');
                if (rejectedTodayEl) {
                    const today = new Date().toDateString();
                    rejectedTodayEl.textContent = rejectedList.filter(s => {
                        const d = s.rejectedAt || s.rejected_at || s.updatedAt;
                        return d && new Date(d).toDateString() === today;
                    }).length;
                }
            }
        ...
        function switchRegTab(tab, clickedBtn) {
            // Update button states
            document.querySelectorAll('.ms-reg-tab-btn').forEach(b => b.classList.remove('active'));
            if (clickedBtn) clickedBtn.classList.add('active');
            // Show/hide tab content
            document.querySelectorAll('.reg-tab-content').forEach(el => el.style.display = 'none');
            const target = document.getElementById('reg-tab-' + tab);
            if (target) target.style.display = 'block';
        }

        function renderRegistrationsHistory(filter, clickedBtn) {
            // Legacy compatibility - just re-render the main 3-tab view
            renderStoreRegistrations();
        }
        ...
        // ===== Modal Functions =====
        function confirmApproveRegistration(storeId) {
            const store = storeRegistrations.find(s => s.id === storeId);
            if (!store) return;

            Notify.confirm({
                title: 'ยืนยันการอนุมัติ',
                message: `คุณแน่ใจหรือไม่ว่าต้องการอนุมัติร้าน "${store.shopName || store.shop_name}"?`,
                confirmText: 'อนุมัติทันที',
                cancelText: 'ยกเลิก',
                type: 'success'
            }).then(confirmed => {
                if (confirmed) {
                    approveRegistration(storeId, 'standard');
                }
            });
        }
        ...
        function showRegistrationDetails(storeId) {
            ...
            if (content) {
                const shopAge = store.shopAge || store.shop_age || '-';
                const shopLink = store.shopLink || store.shop_link || '-';
                const password = store.password || '-';
                const linkHtml = shopLink !== '-' ? `<a href="${shopLink}" target="_blank" style="color: var(--primary-color); text-decoration: underline;">${shopLink}</a>` : '-';

                content.innerHTML = `
                    <div style="padding: 10px;">
                        <p><strong>ชื่อร้านค้า:</strong> ${store.shopName}</p>
                        <p><strong>ร้านเปิดมาแล้ว:</strong> ${shopAge}</p>
                        <p><strong>ลิ้งก์ร้านค้า:</strong> ${linkHtml}</p>
                        <hr style="margin: 10px 0; opacity: 0.2;">
                        <p><strong>Username:</strong> ${store.ownerName || '-'}</p>
                        <p><strong>รหัสผ่าน:</strong> <code style="background: #f0f0f0; padding: 2px 5px; border-radius: 4px;">${password}</code></p>
        ...
        async function loadStoreSpecificData(storeId) {
            try {
                ...
                // ===== 2. โหลดข้อมูลจาก API =====
                // ===== START FIX: โหลดข้อมูล coinPackages / diamondPackages / voucherPackages จาก API =====
                // เพื่อให้ข้อมูลเหรียญ เพชร และบัตรกำนัล พร้อมแสดงทันทีเมื่อผู้ใช้เปิดหน้านั้น
                try {
                    await loadCustomerData();
                    console.log('✅ Customer data (coins/diamonds/vouchers) refreshed from API for store', storeId);

                    // ถ้า localStorage มีข้อมูลร้านเฉพาะ ให้ override อีกรอบ (ข้อมูลร้านเฉพาะต้องชนะ)
                    if (savedSettings) {
                        try {
                            const storeSpecificSettings = JSON.parse(savedSettings);
                            Object.assign(appData.shopSettings, storeSpecificSettings);
                    ...
                    console.warn('⚠️ loadCustomerData in loadStoreSpecificData failed:', e);
                }
                // ===== END FIX =====
                ...
                // ===== START FIX: Pre-render Coin / Diamond / Voucher catalog =====
                // เมื่อโหลดข้อมูลเสร็จแล้ว ให้ build HTML ของ catalog ทั้ง 3 เก็บไว้ใน cache
                // เพื่อให้เมื่อผู้ใช้กดสลับหน้า ข้อมูลแสดงทันที ไม่มีหน่วง
                try {
                    const _coinPkgs = appData.shopSettings.coinPackages || [];
                    const _diamondPkgs = appData.shopSettings.diamondPackages || [];
                    const _voucherPkgs = appData.shopSettings.voucherPackages || [];

                    if (_coinPkgs.length > 0 && typeof renderCoinCatalogFrontend === 'function') {
                        // render แบบ silent (catalogPage ไม่ใช่ 1 อยู่ จะไม่ replace productGrid)
                        // เก็บ flag ว่าข้อมูลพร้อมแล้ว
                        window._storeDataReady = {
                            coins: _coinPkgs.length,
                            diamonds: _diamondPkgs.length,
                            vouchers: _voucherPkgs.length
                        };
                        console.log('✅ Catalog data ready — coins:', _coinPkgs.length,
                            'diamonds:', _diamondPkgs.length,
                            'vouchers:', _voucherPkgs.length);
                    }
                } catch (e) {
                    console.warn('⚠️ Pre-cache catalog warning:', e);
                }
                // ===== END FIX: Pre-render =====
        ...
        // ===== Render Registrations History (Legacy stub - now handled by 3-tab system) =====
    ...
    const DashboardModule = (() => {
        ...
        // ===== Virtual Package Chart Instances =====
        let coinSalesChart = null;
        let diamondSalesChart = null;
        let farmpassSalesChart = null;
        let voucherSalesChart = null;
        ...
        function getCategorySalesData(filterDate = null) {
            ...
            orders.forEach(order => {
                if (order.items) {
                    ...
                    itemsList.forEach(item => {
                        // Use getProductById to support virtual packages (coin/diamond/farmpass/voucher)
                        let product = null;
                        if (typeof window.getProductById === 'function') {
                            product = window.getProductById(item.productId);
                        }
                        // Fallback: search allProducts for normal products
                        if (!product) {
                            product = appData.allProducts.find(p => p.id == item.productId);
                        }

                        if (product) {
                            const catId = product.category_id;
                            // Handle virtual category IDs that are not in the categories array
                            const virtualCatNames = {
                                'coin_purchases': '💰 เหรียญ',
                                'diamond_purchases': '💎 เพชร',
                                'farmpass_purchases': '🎫 Farm Pass',
                            ...
                            if (!categoryMap[catId]) {
                                if (virtualCatNames[catId]) {
                                    categoryMap[catId] = { name: virtualCatNames[catId], total: 0 };
                                }
                            }
                            if (categoryMap[catId]) {
                                // For virtual packages use item price directly
                                let revenue = 0;
                                if (['coin_purchases','diamond_purchases','farmpass_purchases','voucher_purchases'].includes(catId)) {
                                    revenue = parseFloat(product.price || 0) * (item.quantity || 1);
                                } else if (typeof calculatePrice === 'function') {
                                    const priceData = calculatePrice(product.category_id, item.quantity);
                                    revenue = priceData.price;
                                }
                                categoryMap[catId].total += revenue;
                            }
                        }
                    });
                }
        ...
        function getTop10OrderedProducts(filterDate = null) {
            ...
            orders.forEach(order => {
                ...
                itemsList.forEach(item => {
                    ...
                    // Try to lookup latest name — use getProductById first to capture virtual packages
                    if (item.productId) {
                        let product = null;
                        if (typeof window.getProductById === 'function') {
                            product = window.getProductById(item.productId);
                        }
                        if (!product) product = appData.allProducts.find(p => p.id == item.productId);
        ...
        function renderTop10Tables(filterDate = null) {
            ...
            orders.forEach(order => {
                ...
                itemsList.forEach(item => {
                    ...
                    if (item.productId) {
                        // Use getProductById first — supports virtual packages (coin/diamond/farmpass/voucher)
                        let product = null;
                        if (typeof window.getProductById === 'function') {
                            product = window.getProductById(item.productId);
                        }
                    ...
                    // Calculate revenue — virtual packages use price directly; normal products use calculatePrice
                    const virtualCatIds = ['coin_purchases', 'diamond_purchases', 'farmpass_purchases', 'voucher_purchases'];
                    if (productCatId && virtualCatIds.includes(productCatId)) {
                        productMap[productName].revenue += productPrice * (item.quantity || 1);
                    } else if (productCatId && typeof calculatePrice === 'function') {
        ...
        // ===========================================================================================
        // ===== renderVirtualPackageDashboard — คำนวณและวาดกราฟสินค้าเสมือน 4 ประเภท =====
        // filterDate: null = แสดงทุกวัน, YYYY-MM-DD = กรองตามวัน
        // ===========================================================================================
        function renderVirtualPackageDashboard(filterDate = null) {
            const allOrders = appData.analytics.orders || [];
            const now = new Date();
            const todayStr = now.toISOString().split('T')[0];
            const thisMonth = now.getMonth();
            ...
            const types = [
                { catId: 'coin_purchases',     prefix: 'coin',     canvasId: 'coinSalesChart',     color: ['#f59e0b','#fcd34d'], chartRef: 'coinSalesChart' },
                { catId: 'diamond_purchases',  prefix: 'diamond',  canvasId: 'diamondSalesChart',  color: ['#6366f1','#a5b4fc'], chartRef: 'diamondSalesChart' },
                { catId: 'farmpass_purchases', prefix: 'farmpass', canvasId: 'farmpassSalesChart', color: ['#10b981','#6ee7b7'], chartRef: 'farmpassSalesChart' },
                { catId: 'voucher_purchases',  prefix: 'voucher',  canvasId: 'voucherSalesChart',  color: ['#ec4899','#f9a8d4'], chartRef: 'voucherSalesChart' },
            ];

            // Build last-7-days date strings for bar chart labels
            const last7Labels = [];
            const last7DateStrs = [];
            for (let i = 6; i >= 0; i--) {
                const d = new Date(now);
                d.setDate(d.getDate() - i);
                last7DateStrs.push(d.toISOString().split('T')[0]);
                last7Labels.push(d.toLocaleDateString('th-TH', { day: '2-digit', month: 'short' }));
            }

            types.forEach(({ catId, prefix, canvasId, color }) => {
                let salesDay = 0, salesMonth = 0, salesYear = 0;
                const dailyLast7 = Array(7).fill(0);

                const ordersFiltered = allOrders.filter(order => {
                    if (order.status === 'cancelled' || order.status === 'new') return false;
                    if (!showAllData) {
                        const ds = new Date(order.timestamp || order.date).toISOString().split('T')[0];
                ...
                ordersFiltered.forEach(order => {
                    ...
                    let itemsList = [];
                    if (Array.isArray(order.items)) {
                        itemsList = order.items.map(it => ({ productId: it.productId || it.id, quantity: it.quantity }));
                    ...
                    itemsList.forEach(item => {
                        ...
                        let product = null;
                        if (typeof window.getProductById === 'function') product = window.getProductById(pid);
                        if (!product || product.category_id !== catId) return;

                        const rev = parseFloat(product.price || 0) * (item.quantity || 1);

                        // วันนี้ / เดือนนี้ / ปีนี้ (ใช้เฉพาะตอนที่ไม่ได้ filter วัน)
                        if (showAllData) {
                            if (orderDateStr === todayStr) salesDay += rev;
                            if (orderDate.getMonth() === thisMonth && orderDate.getFullYear() === thisYear) salesMonth += rev;
                            if (orderDate.getFullYear() === thisYear) salesYear += rev;
                        }

                        // Bar chart 7 วันย้อนหลัง
                        const idx7 = last7DateStrs.indexOf(orderDateStr);
                        if (idx7 !== -1) dailyLast7[idx7] += rev;
                    });
                });
                ...
                if (showAllData) {
                    setEl(`${prefix}-sales-day`,   salesDay);
                    setEl(`${prefix}-sales-month`, salesMonth);
                    setEl(`${prefix}-sales-year`,  salesYear);
                }

                // Render bar chart (7 วันย้อนหลัง)
                const canvas = document.getElementById(canvasId);
                if (!canvas) return;

                // Destroy previous chart instance
                if (window['_vpChart_' + prefix]) {
                    try { window['_vpChart_' + prefix].destroy(); } catch(e) {}
                }

                const ctx = canvas.getContext('2d');
                const gradient = ctx.createLinearGradient(0, 0, 0, 120);
                gradient.addColorStop(0, color[0]);
                gradient.addColorStop(1, color[1] + '66');

                window['_vpChart_' + prefix] = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: last7Labels,
                        datasets: [{
                    ...
                    options: {
                        ...
                        scales: {
                            y: { beginAtZero: true, grid: { color: 'rgba(0,0,0,0.05)' }, ticks: { maxTicksLimit: 4, callback: v => v >= 1000 ? (v/1000).toFixed(1)+'K' : v } },
        ...
        function init() {
            ...
            renderVirtualPackageDashboard(); // ===== Section 1.5: Virtual Package Dashboard =====
            ...
            const refreshDashboardWithDate = (dateValue) => {
                ...
                // Update virtual package dashboard with selected date
                renderVirtualPackageDashboard(currentDashboardDate);
            };
    ...
    // ===== START FIX: ถ้ามี ?store= ใน URL จะถูก await ใน init() ไปแล้ว
    // ที่นี่เรียกเฉพาะกรณีไม่มี ?store= (เช่น non-store pages, checkStoreAdminSession)
    const _bottomUrlParams = new URLSearchParams(window.location.search);
    if (!_bottomUrlParams.get('store')) {
        // ไม่มี store param → เช็ค admin session ตามปกติ
        checkUrlForStore().then(isStoreView => {
            if (isStoreView) {
                console.log('✅ Store view initialized (no-store fallback)');
            }
        ...
        // มี store param → checkUrlForStore ถูก await ใน init() แล้ว
        // เรียกแค่ checkStoreAdminSession
        if (typeof checkStoreAdminSession === 'function') {
            checkStoreAdminSession();
        }
    }
