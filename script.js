document.addEventListener('DOMContentLoaded', () => {
    ...
    const setLanguage = (lang) => {
        ...
        // Ensure order buttons are updated with icons and new text
    ...
    const checkOrderValidation = () => {
        ...
        // Update button contents with product icons
        const uniqueProductIds = Object.keys(appData.cart).filter(id => appData.cart[id] > 0);
        let iconsHTML = '';
        if (uniqueProductIds.length > 0) {
            iconsHTML = '<div class="btn-cart-icons">';
            // Show up to 3 unique items for visual stack
            uniqueProductIds.slice(0, 3).forEach(id => {
                const prod = appData.allProducts.find(p => p.id == id);
                if (prod && prod.icon) {
                    iconsHTML += `<img src="${prod.icon}" class="btn-cart-icon" alt="">`;
                }
            });
            iconsHTML += '</div>';
        }

        const viewBtnText = translations[lang].viewOrderBtn;
        const confirmBtnText = translations[lang].confirmOrderBtn;

        viewOrderBtn.innerHTML = `${iconsHTML} <span>${viewBtnText}</span>`;
        confirmOrderBtn.innerHTML = `${iconsHTML} <span>${confirmBtnText}</span>`;
