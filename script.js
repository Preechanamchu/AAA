document.addEventListener('DOMContentLoaded', () => {
    ...
    let virtualCoinSalesChart, virtualDiamondSalesChart, virtualFarmpassSalesChart, virtualVoucherSalesChart;
    ...
    const getTopSellingItems = (period) => {
        ...
        ordersToAnalyze.forEach(order => {
            for (const prodId in order.items) {
                const product = window.getProductById(prodId);
                if (product) {
    ...
    const renderDashboard = () => {
        ...
        // Virtual Packages Dashboard
        renderVirtualPackageDashboard(confirmedOrders);
    };

    const renderVirtualPackageDashboard = (confirmedOrders) => {
        const today = new Date(), currentMonth = today.getMonth(), currentYear = today.getFullYear();
        const todayStr = today.toISOString().slice(0, 10);
        
        const stats = {
            coin: { day: 0, month: 0, year: 0, chart: Array(7).fill(0) },
            diamond: { day: 0, month: 0, year: 0, chart: Array(7).fill(0) },
            farmpass: { day: 0, month: 0, year: 0, chart: Array(7).fill(0) },
            voucher: { day: 0, month: 0, year: 0, chart: Array(7).fill(0) }
        };
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            daysLabels.push(d.toLocaleDateString('th-TH', { day: '2-digit', month: 'short' }));
        }

        confirmedOrders.forEach(order => {
            const orderDate = new Date(order.timestamp);
            const isToday = order.timestamp.startsWith(todayStr);
            const isThisMonth = orderDate.getFullYear() === currentYear && orderDate.getMonth() === currentMonth;
            const isThisYear = orderDate.getFullYear() === currentYear;
            
            // Calculate Day Index for chart (0 is oldest, 6 is today)
            const diffTime = Math.abs(today - orderDate);
            const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
            const chartIndex = 6 - diffDays;

            for (const prodId in order.items) {
                const qty = order.items[prodId];
                const product = window.getProductById(prodId);
                
                if (product) {
                    let type = null;
                    if (product.category_id === 'coin_purchases' || product.isCoin) type = 'coin';
                    else if (product.category_id === 'diamond_purchases' || product.isDiamond) type = 'diamond';
                    else if (product.category_id === 'farmpass_purchases' || product.isFarmPass) type = 'farmpass';
                    else if (product.category_id === 'voucher_purchases' || product.isVoucher) type = 'voucher';

                    if (type) {
                        const revenue = product.price * qty;
                        if (isToday) stats[type].day += revenue;
                        if (isThisMonth) stats[type].month += revenue;
                        if (isThisYear) stats[type].year += revenue;
                        if (chartIndex >= 0 && chartIndex <= 6) {
                            stats[type].chart[chartIndex] += revenue;
                        }
                    }
                }
            }
        ...
        const types = ['coin', 'diamond', 'farmpass', 'voucher'];
        types.forEach(type => {
            const elDay = document.getElementById(`${type}-sales-day`);
            const elMonth = document.getElementById(`${type}-sales-month`);
            const elYear = document.getElementById(`${type}-sales-year`);
            if(elDay) elDay.textContent = `${stats[type].day.toLocaleString()} ฿`;
            if(elMonth) elMonth.textContent = `${stats[type].month.toLocaleString()} ฿`;
            if(elYear) elYear.textContent = `${stats[type].year.toLocaleString()} ฿`;
        });

        // Chart definitions
        const chartDefs = [
            { id: 'coinSalesChart', var: 'virtualCoinSalesChart', label: 'ยอดขายเหรียญ (บาท)', data: stats.coin.chart, color: '#f1c40f' },
            { id: 'diamondSalesChart', var: 'virtualDiamondSalesChart', label: 'ยอดขายเพชร (บาท)', data: stats.diamond.chart, color: '#00bcd4' },
            { id: 'farmpassSalesChart', var: 'virtualFarmpassSalesChart', label: 'ยอดขาย Farm Pass (บาท)', data: stats.farmpass.chart, color: '#4caf50' },
            { id: 'voucherSalesChart', var: 'virtualVoucherSalesChart', label: 'ยอดขายบัตรกำนัล (บาท)', data: stats.voucher.chart, color: '#e91e63' }
        ];

        chartDefs.forEach(def => {
            if (window[def.var]) window[def.var].destroy();
            const ctx = document.getElementById(def.id);
            if (!ctx) return;
            window[def.var] = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: daysLabels,
                    datasets: [{
                        label: def.label,
                        data: def.data,
                        backgroundColor: def.color + '40', // 25% opacity
    ...
    const populateProductSelect = () => {
        ...
        let allItems = [...appData.allProducts];
        
        // Add virtual packages
        (appData.shopSettings.coinPackages || []).forEach(p => allItems.push(window.getProductById(p.id)));
        (appData.shopSettings.diamondPackages || []).forEach(p => allItems.push(window.getProductById(p.id)));
        (appData.shopSettings.farmPassPackages || []).forEach(p => allItems.push(window.getProductById(p.id)));
        (appData.shopSettings.voucherPackages || []).forEach(p => allItems.push(window.getProductById(p.id)));

        // Remove undefined items and sort by name
        allItems = allItems.filter(Boolean).sort((a, b) => a.name.localeCompare(b.name));
    ...
    const renderProductSalesChartAdvanced = (productId = 'all') => {
        ...
        const productName = productId === 'all' ? 'ทั้งหมด' : (window.getProductById(productId)?.name || 'สินค้า');
