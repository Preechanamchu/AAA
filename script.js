document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadCustomerData = async () => {
        try {
            ...
            // PRELOAD ALL ICONS IN BACKGROUND
            preloadShopIcons();
    ...
    const loadAdminData = async () => {
        try {
            ...
            // PRELOAD ALL ICONS IN BACKGROUND (Admin context)
            preloadShopIcons();
    ...
    /**
     * Preloads all shop icons and product images into the browser cache
     * to eliminate loading delays when switching categories.
     */
    const preloadShopIcons = () => {
        try {
            console.log('🚀 Starting background icon preloading...');
            const urls = new Set();

            // 1. Categories and Products
            (appData.categories || []).forEach(c => { if(c.icon) urls.add(c.icon); });
            (appData.allProducts || []).forEach(p => { if(p.icon) urls.add(p.icon); });

            // 2. Virtual Packages
            if (appData.shopSettings) {
                (appData.shopSettings.coinPackages || []).forEach(p => { if(p.icon) urls.add(p.icon); });
                (appData.shopSettings.diamondPackages || []).forEach(p => { if(p.icon) urls.add(p.icon); });
                (appData.shopSettings.voucherPackages || []).forEach(p => { if(p.icon) urls.add(p.icon); });
                (appData.shopSettings.farmPassPackages || []).forEach(p => { 
                    if(p.icon) urls.add(p.icon);
                    if(p.image) urls.add(p.image);
                });
                
                // Shop Visual Elements
                if(appData.shopSettings.logo) urls.add(appData.shopSettings.logo);
                if(appData.shopSettings.loadingGif) urls.add(appData.shopSettings.loadingGif);
            }
            ...
            urls.forEach(url => {
                ...
                img.onload = img.onerror = () => {
                    ...
                    if (loadedCount === urls.size) {
                        console.log(`✅ Finished preloading ${urls.size} icons.`);
                    }
                };
                img.src = url;
            });
            
            console.log(`📦 Background loading ${urls.size} unique images...`);
        } catch (e) {
            console.warn('⚠️ Icon preloading error:', e);
        }
    };
    ...
    let coinSalesChart, diamondSalesChart, farmpassSalesChart, voucherSalesChart;
    ...
    const renderDashboard = () => {
        ...
        // ===== START: Virtual Package Sales (Coins/Diamonds/FarmPass/Vouchers) =====
        const coinPkgIds = new Set((appData.shopSettings.coinPackages || []).map(p => p.id));
        const diamondPkgIds = new Set((appData.shopSettings.diamondPackages || []).map(p => p.id));
        const farmPassPkgIds = new Set((appData.shopSettings.farmPassPackages || []).map(p => p.id));
        const voucherPkgIds = new Set((appData.shopSettings.voucherPackages || []).map(p => p.id));

        const coinPkgMap = {};
        (appData.shopSettings.coinPackages || []).forEach(p => { coinPkgMap[p.id] = p; });
        const diamondPkgMap = {};
        (appData.shopSettings.diamondPackages || []).forEach(p => { diamondPkgMap[p.id] = p; });
        const farmPassPkgMap = {};
        (appData.shopSettings.farmPassPackages || []).forEach(p => { farmPassPkgMap[p.id] = p; });
        const voucherPkgMap = {};
        (appData.shopSettings.voucherPackages || []).forEach(p => { voucherPkgMap[p.id] = p; });

        // Helper: calculate virtual sales from a list of orders
        const calcVirtualSales = (orders) => {
            let coin = 0, diamond = 0, farmpass = 0, voucher = 0;
            orders.forEach(order => {
                let items = order.items;
                if (typeof items === 'string') { try { items = JSON.parse(items); } catch(e) { return; } }
                for (const prodId in items) {
                    const qty = items[prodId];
                    if (coinPkgMap[prodId]) coin += (coinPkgMap[prodId].price || 0) * qty;
                    else if (diamondPkgMap[prodId]) diamond += (diamondPkgMap[prodId].price || 0) * qty;
                    else if (farmPassPkgMap[prodId]) farmpass += (farmPassPkgMap[prodId].price || 0) * qty;
                    else if (voucherPkgMap[prodId]) voucher += (voucherPkgMap[prodId].price || 0) * qty;
                }
            });
            return { coin, diamond, farmpass, voucher };
        };

        const vSalesToday = calcVirtualSales(ordersToday);
        const vSalesMonth = calcVirtualSales(ordersInMonth);
        const vSalesYear = calcVirtualSales(ordersInYear);

        // Update stat card values
        const setEl = (id, val) => { const el = document.getElementById(id); if (el) el.textContent = `${val.toLocaleString()} บาท`; };
        setEl('coin-sales-daily', vSalesToday.coin);
        setEl('coin-sales-monthly', vSalesMonth.coin);
        setEl('coin-sales-yearly', vSalesYear.coin);
        setEl('diamond-sales-daily', vSalesToday.diamond);
        setEl('diamond-sales-monthly', vSalesMonth.diamond);
        setEl('diamond-sales-yearly', vSalesYear.diamond);
        setEl('farmpass-sales-daily', vSalesToday.farmpass);
        setEl('farmpass-sales-monthly', vSalesMonth.farmpass);
        setEl('farmpass-sales-yearly', vSalesYear.farmpass);
        setEl('voucher-sales-daily', vSalesToday.voucher);
        setEl('voucher-sales-monthly', vSalesMonth.voucher);
        setEl('voucher-sales-yearly', vSalesYear.voucher);

        // Render daily charts (last 7 days) for each virtual type
        const renderVirtualDailyChart = (chartVar, canvasId, label, color, pkgMap) => {
            ...
            for (let i = 6; i >= 0; i--) {
                ...
                labels7.push(d.toLocaleDateString('th-TH', { day: '2-digit', month: 'short' }));
                const dayOrders = confirmedOrders.filter(o => o.timestamp.startsWith(ds));
                let dayTotal = 0;
                dayOrders.forEach(order => {
                    let items = order.items;
                    if (typeof items === 'string') { try { items = JSON.parse(items); } catch(e) { return; } }
                    for (const prodId in items) {
                        if (pkgMap[prodId]) dayTotal += (pkgMap[prodId].price || 0) * items[prodId];
                    }
                });
                data7.push(dayTotal);
            }
        ...
        coinSalesChart = renderVirtualDailyChart(coinSalesChart, 'coinSalesChart', 'ยอดขายเหรียญ', '#ffa502', coinPkgMap);
        diamondSalesChart = renderVirtualDailyChart(diamondSalesChart, 'diamondSalesChart', 'ยอดขายเพชร', '#3a7bd5', diamondPkgMap);
        farmpassSalesChart = renderVirtualDailyChart(farmpassSalesChart, 'farmpassSalesChart', 'ยอดขาย Farm Pass', '#4facfe', farmPassPkgMap);
        voucherSalesChart = renderVirtualDailyChart(voucherSalesChart, 'voucherSalesChart', 'ยอดขายบัตรกำนัล', '#6c5ce7', voucherPkgMap);
        // ===== END: Virtual Package Sales =====
