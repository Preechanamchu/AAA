document.addEventListener('DOMContentLoaded', () => {
    ...
    let appData = {
        ...
        shopSettings: {
            ...
            coinPackages: [],
            voucherPackages: [
                { id: 'vouch_1', vouchers: 50, price: 49, icon: '' },
                { id: 'vouch_2', vouchers: 100, price: 99, icon: '' },
                { id: 'vouch_3', vouchers: 200, price: 189, icon: '' },
                { id: 'vouch_4', vouchers: 300, price: 259, icon: '' },
                { id: 'vouch_5', vouchers: 500, price: 359, icon: '' },
                { id: 'vouch_6', vouchers: 1000, price: 459, icon: '' },
                { id: 'vouch_7', vouchers: 2000, price: 859, icon: '' },
                { id: 'vouch_8', vouchers: 3000, price: 1399, icon: '' },
                { id: 'vouch_9', vouchers: 5000, price: 1899, icon: '' },
                { id: 'vouch_10', vouchers: 10000, price: 3499, icon: '' }
            ],
    ...
    const translations = {
        th: {
            ...
            manageCategoriesTitle: "จัดการหมวดหมู่", manageCoinPackagesTitle: "เติมเหรียญ", manageDiamondPassTitle: "เพชร & Pass", manageVouchersTitle: "บัตรกำนัล", categoryNameLabel: "ชื่อหมวดหมู่", categoryNameEnLabel: "ชื่อหมวดหมู่ (English)", categoryIconLabel: "ไอค่อนหมวดหมู่", minOrderLabel: "จำนวนสั่งซื้อขั้นต่ำ",
        ...
        en: {
            ...
            manageCategoriesTitle: "Manage Categories", manageCoinPackagesTitle: "Add Coins", manageDiamondPassTitle: "Diamond & Pass", manageVouchersTitle: "Vouchers", categoryNameLabel: "Category Name", categoryNameEnLabel: "Category Name (English)", categoryIconLabel: "Category Icon", minOrderLabel: "Minimum Order Quantity",
    ...
    window.getProductById = (productId) => {
        ...
        if(!product) {
            ...
            if (voucherPkg) {
                return {
                    id: voucherPkg.id,
                    name: `${voucherPkg.vouchers.toLocaleString()} 🎟️`,
                    name_en: `${voucherPkg.vouchers.toLocaleString()} 🎟️`,
                    level: '',
    ...
    const checkOrderValidation = () => {
        ...
        for (const categoryId of sortedCategoryIds) {
            ...
            if (categoryId === 'coin_purchases' || categoryId === 'diamond_purchases' || categoryId === 'farmpass_purchases') continue;
            // ===== END: Skip =====
                ...
                if (product) {
                    if (!itemsByCategory[product.category_id]) {
                        itemsByCategory[product.category_id] = {
                            items: [],
                            name: appData.categories.find(c => c.id === product.category_id)?.name || (product.isCoin ? 'แพ็กเกจเหรียญ' : product.isDiamond ? 'เติมเพชร' : product.isFarmPass ? 'Farm Pass' : product.isVoucher ? 'บัตรกำนัล' : 'Unknown'),
        ...
        for (const catId in itemsByCategory) {
            ...
            if (catId === 'coin_purchases') {
                ...
                const totalVouchers = categoryData.items.reduce((sum, item) => {
                    ...
                    return sum + ((vPkg ? vPkg.vouchers : 0) * item.quantity);
                }, 0);
                summaryText += `${catName}: ${totalVouchers.toLocaleString()} 🎟️\n`;
            } else {
    ...
    const createConfirmOrderHTML = (orderNumber) => {
        ...
        for (const catId in itemsByCategory) {
            ...
            if (catId === 'coin_purchases') {
                ...
                quantityDisplay = `${totalVouchers.toLocaleString()} 🎟️`;
            } else {
    ...
    cartDetails.addEventListener('click', (e) => {
        ...
        // Ensure string IDs remain strings (coins, diamonds, farm pass)
        if (!productId.startsWith('coin_') && !productId.startsWith('diamond_') && !productId.startsWith('farmpass_')) {
    ...
    window.addVoucherToCart = (id) => {
        ...
        // Single-select: Clear previous voucher packages
        const allPkgIds = (appData.shopSettings.voucherPackages || []).map(p => p.id);
        allPkgIds.forEach(pkgId => {
            if (appData.cart[pkgId]) delete appData.cart[pkgId];
        });
        ...
        checkOrderValidation();
        if (typeof renderVoucherCatalogFrontend === 'function') renderVoucherCatalogFrontend();
        Notify.success('เลือกบัตรกำนัลแล้ว', `${pkg.vouchers.toLocaleString()} 🎟️ ถูกเพิ่มลงบัญชีสั่งซื้อ`);
    };
    ...
    window.renderVoucherCatalogFrontend = () => {
        ...
        // Single-select: Determine which voucher package is currently selected
        const allPkgIds = packages.map(p => p.id);
        const selectedPkgId = allPkgIds.find(id => appData.cart[id] && appData.cart[id] > 0) || null;

        const container = document.createElement('div');
        container.className = 'voucher-catalog-container';
        container.style.cssText = 'width: 100%; grid-column: 1/-1; display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;';

        const sortedPkgs = [...packages].sort((a,b) => a.vouchers - b.vouchers);

        container.innerHTML = sortedPkgs.map(pkg => {
            const isSelected = selectedPkgId === pkg.id;
            const isDisabled = selectedPkgId && selectedPkgId !== pkg.id;
            const iconUrl = pkg.icon || '';
            const iconHtml = iconUrl ? `<img src="${iconUrl}" alt="voucher" style="width: 36px; height: 36px; border-radius: 6px; object-fit: cover;">` : `<span style="font-size: 1.5rem;">🎟️</span>`;
            
            let buttonHtml = '';
            if (isSelected) {
                buttonHtml = `<span style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; background: #ff9800; border-radius: 50%; color: #fff; font-size: 1.2rem; box-shadow: 0 2px 8px rgba(255,152,0,0.3); animation: coinCheckPop 0.3s ease;">✓</span>`;
            } else if (isDisabled) {
                buttonHtml = `<button class="btn btn-small" disabled style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem; opacity: 0.4; cursor: not-allowed; background: #ccc; color: #888; border: none;">เลือก</button>`;
            } else {
                buttonHtml = `<button class="btn btn-primary btn-small" onclick="addVoucherToCart('${pkg.id}')" style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem; background: #ff9800; border: none;">เลือก</button>`;
            }
            
            return `
            <div class="voucher-package-item" style="display: flex; justify-content: space-between; align-items: center; background: ${isSelected ? '#fff3e0' : '#fff'}; padding: 15px; border-radius: 12px; box-shadow: 0 3px 10px rgba(0,0,0,0.06); border: ${isSelected ? '2px solid #ff9800' : '1px solid #f0f0f0'}; transition: all 0.3s ease; ${isDisabled ? 'opacity: 0.6;' : ''}">
                <div style="display: flex; align-items: center; gap: 12px;">
                    ${iconHtml}
                    <div style="display: flex; flex-direction: column;">
                        <span style="font-weight: bold; color: #e65100; font-size: 1.2rem;">${pkg.vouchers.toLocaleString()} ใบ</span>
                    </div>
                </div>
                <div style="display: flex; align-items: center; gap: 15px;">
                    <span style="color: #424242; font-weight: bold; font-size: 1.1rem;">${pkg.price.toLocaleString()} บาท</span>
                    ${buttonHtml}
                </div>
            </div>
            `;
    ...
    document.getElementById('voucher-package-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const submitBtn = document.getElementById('submit-voucher-btn');
        submitBtn.disabled = true;
        submitBtn.textContent = 'กำลังบันทึก...';

        const vouchers = parseInt(document.getElementById('voucher-amount').value);
        const price = parseFloat(document.getElementById('voucher-price').value);
        const icon = document.getElementById('voucher-icon-url').value.trim();

        if (isNaN(vouchers) || isNaN(price)) {
            Notify.error('Error', 'กรุณากรอกข้อมูลตัวเลขให้ถูกต้อง');
            submitBtn.disabled = false;
            submitBtn.textContent = 'เพิ่มแพ็กเกจบัตรกำนัล';
            return;
        }
        ...
        if (editingVoucherPkgId) {
            const index = appData.shopSettings.voucherPackages.findIndex(p => p.id === editingVoucherPkgId);
            if (index !== -1) {
                appData.shopSettings.voucherPackages[index] = { id: editingVoucherPkgId, vouchers, price, icon };
            }
            editingVoucherPkgId = null;
        } else {
            const newPkg = { id: 'vouch_' + Date.now(), vouchers, price, icon };
        ...
        document.getElementById('submit-voucher-btn').textContent = 'เพิ่มแพ็กเกจบัตรกำนัล';
        document.getElementById('cancel-voucher-edit-btn').style.display = 'none';

        await saveState();
        if (typeof renderAdminVoucherPackages === 'function') renderAdminVoucherPackages();
        
        Notify.success('สำเร็จ', 'บันทึกข้อมูลบัตรกำนัลสำเร็จ');
        submitBtn.disabled = false;
    });
    ...
    window.editVoucherPackage = (id) => {
        ...
        if (pkg) {
            ...
            document.getElementById('voucher-amount').value = pkg.vouchers;
    ...
    window.deleteVoucherPackage = async (id) => {
        if (confirm('คุณแน่ใจหรือไม่ว่าต้องการลบแพ็กเกจบัตรกำนัลนี้?')) {
    ...
    window.renderAdminVoucherPackages = () => {
        ...
        if (!appData.shopSettings.voucherPackages || appData.shopSettings.voucherPackages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-color);">ไม่มีข้อมูลแพ็กเกจบัตรกำนัล</td></tr>';
            return;
        }

        appData.shopSettings.voucherPackages.forEach(pkg => {
            const tr = document.createElement('tr');
            const iconPreview = pkg.icon 
                ? `<img src="${pkg.icon}" alt="icon" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover;">` 
                : `<span style="color: #ccc;">-</span>`;
            tr.innerHTML = `
                <td style="text-align: center;">${iconPreview}</td>
                <td style="text-align: right; color: #ff9800; font-weight: bold;">${pkg.vouchers.toLocaleString()} 🎟️</td>
                <td style="text-align: right;">${pkg.price.toLocaleString()} บาท</td>
                <td style="text-align: center;">
                    <div style="display: flex; gap: 5px; justify-content: center;">
    ...
    document.getElementById('toggle-voucher-form-btn').addEventListener('click', (e) => {
        const form = document.getElementById('voucher-package-form');
        const isHidden = form.style.display === 'none';
        form.style.display = isHidden ? 'block' : 'none';
        e.target.textContent = isHidden ? 'ซ่อน' : 'แสดง';
    });
