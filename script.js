document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('swap-catalog-btn').addEventListener('click', function() {
        ...
        if (productGrid) {
            // ===== START FIX: สำหรับหน้า coin/diamond/voucher ให้แสดงทันที ไม่รอ fade =====
            // กรณีเป็นหน้าสินค้าปกติ (page 0) ยังคง fade transition ไว้
            const isCatalogPage = catalogPage === 1 || catalogPage === 2 || catalogPage === 3;

            if (isCatalogPage) {
                // ซ่อน controls wrapper ก่อนเลย
                if (controlsWrapper) {
                ...
                // Render ข้อมูลทันที (ไม่รอ setTimeout)
                if (catalogPage === 1) {
                    renderCoinCatalogFrontend();
            ...
            } // ===== END: else block for product page (isCatalogPage === false) =====
        }
    });
        ...
        Notify.confirm(
            ...
            (result) => {
                if (result) {
                    ...
                    renderProducts();
                    checkOrderValidation();
                    // Re-render virtual catalogs if visible
                    if (catalogPage === 1 && typeof renderCoinCatalogFrontend === 'function') {
                        renderCoinCatalogFrontend();
                    } else if (catalogPage === 2 && typeof renderDiamondPassCatalog === 'function') {
                        renderDiamondPassCatalog();
