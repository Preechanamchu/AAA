document.addEventListener('DOMContentLoaded', () => {
    ...
    const createConfirmOrderSummary = (orderNumber) => {
                ...
                if (product) {
                    if (!itemsByCategory[product.category_id]) {
                        itemsByCategory[product.category_id] = {
                            items: [],
                            name: appData.categories.find(c => c.id === product.category_id)?.name || (product.isCoin ? 'แพ็กเกจเหรียญ' : product.isDiamond ? 'เติมเพชร' : product.isFarmPass ? 'Farm Pass' : 'Unknown'),
                            name_en: appData.categories.find(c => c.id === product.category_id)?.name_en || (product.isCoin ? 'Coin Packages' : product.isDiamond ? 'Diamonds' : product.isFarmPass ? 'Farm Pass' : 'Unknown')
                        };
                    }
        ...
        Object.keys(itemsByCategory).forEach(catId => {
            ...
            categoryData.items.forEach(item => {
                ...
                if (item.isCoin || item.isDiamond || item.isFarmPass) {
    ...
    window.renderVoucherCatalogFrontend = () => {
        ...
        headerDiv.style.cssText = 'grid-column: 1 / -1; text-align: center; margin-bottom: 10px; margin-top: 10px;';
        headerDiv.innerHTML = `<h2 style="color: var(--primary-color); font-size: 1.5rem; text-shadow: 1px 1px 2px rgba(0,0,0,0.1);">" บัตรกำนัล "</h2>`;
        ...
        packages.forEach(pkg => {
            ...
            let btnOrCheck = '';
            if (isSelected) {
                btnOrCheck = `<span class="voucher-selected-check" style="color: #27ae60; font-size: 1.4rem; font-weight: bold; margin-left: auto;">✓</span>`;
            }
            ...
            itemDiv.style.cssText = `
                ...
                border-radius: 12px; padding: 15px; cursor: ${isDisabled ? 'not-allowed' : 'pointer'}; 
                opacity: ${isDisabled ? '0.5' : '1'}; transition: all 0.2s ease;
                box-shadow: 0 2px 5px rgba(0,0,0,0.05);
            `;
            if (!isDisabled) {
                itemDiv.onclick = () => addVoucherToCart(pkg.id);
            }
            ...
            itemDiv.innerHTML = `
                <div style="display: flex; align-items: center; flex: 1;">
                    ...
                    <div style="display: flex; flex-direction: column; text-align: left;">
                        <span style="font-weight: bold; font-size: 1.1rem; color: #333;">${pkg.quantity.toLocaleString()} ใบ</span>
