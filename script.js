document.addEventListener('DOMContentLoaded', () => {
    ...
    window.renderVoucherCatalogFrontend = () => {
        ...
        // ใช้ auto-fit และ minmax เพื่อให้แสดง 1 คอลัมน์บนมือถือ และ 2 คอลัมน์ขึ้นไปบนจอใหญ่
        container.style.cssText = 'width: 100%; grid-column: 1/-1; display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px; padding-bottom: 20px;';
        ...
        packages.forEach(pkg => {
            ...
            const iconUrl = pkg.icon || '';
            const iconHtml = iconUrl ? `<img src="${iconUrl}" alt="voucher" style="width: 32px; height: 32px; border-radius: 6px; object-fit: cover; margin-right: 8px;">` : '';
            ...
            if (isSelected) {
                ...
                btnOrCheck = `<button class="btn btn-small" disabled style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem; opacity: 0.4; cursor: not-allowed; background: #ccc; color: #888; border: none;">เลือก</button>`;
            } else {
            ...
            itemDiv.style.cssText = `
                ...
                background: ${isSelected ? 'linear-gradient(135deg, #f0fff4, #e6ffed)' : '#ffffff'}; 
                border: 2px solid ${isSelected ? '#27ae60' : '#f0f0f0'}; 
                border-radius: 12px; padding: 12px; transition: all 0.2s ease;
                box-shadow: 0 4px 10px rgba(0,0,0,0.05); ${isDisabled ? 'opacity: 0.5;' : ''}
            ...
            itemDiv.innerHTML = `
                <div style="display: flex; align-items: center; flex: 1; gap: 8px; min-width: 0;">
                    ...
                    <div style="display: flex; flex-direction: column; text-align: left; min-width: 0; overflow: hidden;">
                        <span style="font-weight: bold; font-size: 1rem; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.quantity.toLocaleString()} ใบ</span>
                        <span style="color: #e74c3c; font-weight: bold; font-size: 0.9rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${pkg.price.toLocaleString()} บาท</span>
                    </div>
                </div>
                ${btnOrCheck}
            `;
