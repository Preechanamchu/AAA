document.addEventListener('DOMContentLoaded', () => {
    ...
    const translations = {
        th: {
            ...
            manageCategoriesTitle: "จัดการหมวดหมู่", manageCoinPackagesTitle: "เติมเหรียญ", categoryNameLabel: "ชื่อหมวดหมู่", categoryNameEnLabel: "ชื่อหมวดหมู่ (English)", categoryIconLabel: "ไอค่อนหมวดหมู่", minOrderLabel: "จำนวนสั่งซื้อขั้นต่ำ",
        ...
        en: {
            ...
            manageCategoriesTitle: "Manage Categories", manageCoinPackagesTitle: "Add Coins", categoryNameLabel: "Category Name", categoryNameEnLabel: "Category Name (English)", categoryIconLabel: "Category Icon", minOrderLabel: "Minimum Order Quantity",
        ...
        if(!product) {
            ...
            if(coinPkg) {
                return {
                    ...
                    level: coinPkg.levelRange, // Stored as "7-10" instead of "เลเวล 7-10" to match standard Level UI
                    price: coinPkg.price,
                    icon: coinPkg.icon || '',
    ...
    const checkOrderValidation = () => {
        ...
        for (const productId in appData.cart) {
            ...
            if (quantity > 0) {
                ...
                if (product) {
                    ...
                    if (product.isCoin) grandTotalPrice += (product.price * quantity);
                }
            }
        }
        ...
        for (const categoryId of sortedCategoryIds) {
            ...
            // ===== COIN: Skip min/max validation for coin packages =====
            if (categoryId === 'coin_purchases') continue;
            // ===== END: COIN =====
    ...
    const createConfirmOrderSummary = (orderNumber) => {
                ...
                if (product) {
                    if (!itemsByCategory[product.category_id]) {
                        itemsByCategory[product.category_id] = {
                            items: [],
                            name: appData.categories.find(c => c.id === product.category_id)?.name || (product.isCoin ? 'แพ็กเกจเหรียญ' : 'Unknown'),
                            name_en: appData.categories.find(c => c.id === product.category_id)?.name_en || (product.isCoin ? 'Coin Packages' : 'Unknown')
                        };
                    }
        ...
        Object.keys(itemsByCategory).forEach(catId => {
            ...
            categoryData.items.forEach(item => {
                ...
                if (item.isCoin) {
                    summaryText += `${item.level} ${prodName} - ${(item.price * item.quantity).toLocaleString()} ${currencySuffix}\n`;
                } else {
        ...
        for (const catId in itemsByCategory) {
            ...
            // ===== COIN QUANTITY DISPLAY: Show total coins instead of piece count =====
            ...
            // ===== END: COIN QUANTITY DISPLAY =====
            totalAllItems += totalQuantity;

            // ===== COIN PRICE: Use coin price directly instead of calculatePrice =====
            if (catId === 'coin_purchases') {
    ...
    const createConfirmOrderHTML = (orderNumber) => {
        ...
        Object.keys(itemsByCategory).forEach(catId => {
            ...
            categoryData.items.forEach(item => {
                ...
                // ===== COIN: Show price instead of quantity for coin packages =====
                let quantityOrPrice = '';
                if (item.isCoin) {
                ...
                html += `
                    <div class="order-visual-item">
                        <div class="order-visual-info">
                            ...
                            <span class="order-item-name">${item.isCoin ? '' : 'LV'}${item.level} ${prodName}</span>
                        </div>
                        ${quantityOrPrice}
                    </div>`;
            });
        ...
        for (const catId in itemsByCategory) {
            ...
            totalAllItems += totalQuantity;
            // ===== COIN PRICE: Use coin price directly =====
            if (catId === 'coin_purchases') {
                grandTotalPrice += categoryData.items.reduce((sum, item) => sum + (item.price * item.quantity), 0);
            } else {
                const priceResult = calculatePrice(parseInt(catId), totalQuantity);
            ...
            if (quantity > 0) {
                ...
                if (product) {
                    if (!itemsByCategory[product.category_id]) {
                        ...
                        itemsByCategory[product.category_id] = {
                            items: [],
                            name: category ? ((lang === 'en' && category.name_en) ? category.name_en : category.name) : (product.isCoin ? (lang === 'en' ? 'Coin Packages' : 'แพ็กเกจเหรียญ') : 'Unknown'),
            ...
            categoryData.items.forEach(item => {
                ...
                itemDiv.innerHTML = `
                    <div class="cart-item-info">
                        <img src="${iconUrl}" class="summary-item-icon" alt="${prodName}">
                        <span class="cart-item-name">${item.isCoin ? '' : 'LV'}${item.level} ${prodName}</span>
                    </div>
    ...
    cartDetails.addEventListener('click', (e) => {
        ...
        if (!productId) return;
        if (!productId.startsWith('coin_')) productId = parseInt(productId);
        ...
        checkOrderValidation();
        renderProducts(searchBox.value.trim());
        // Re-render coin catalog if it's currently showing, to update checkmark/disabled states
        if (isCatalogUpdating && typeof renderCoinCatalogFrontend === 'function') {
    ...
    document.getElementById('swap-catalog-btn').addEventListener('click', function() {
        isCatalogUpdating = !isCatalogUpdating;
        ...
        Notify.confirm(
            ...
            (result) => {
                if (result) {
                    ...
                    renderProducts();
                    checkOrderValidation();
                    // Re-render coin catalog if visible
                    if (isCatalogUpdating && typeof renderCoinCatalogFrontend === 'function') {
                        renderCoinCatalogFrontend();
                    }
