document.addEventListener('DOMContentLoaded', () => {
    ...
    const init = async () => {
        ...
        const urlParams = new URLSearchParams(window.location.search);
        const hasStoreParam = !!urlParams.get('store');

        // Launch critical data fetching in parallel for maximum speed
        const initPromises = [
            logTraffic(),
            loadCustomerData()
        ];

        // If we have a store parameter, fetch store data in parallel too
        if (hasStoreParam) {
            initPromises.push(checkUrlForStore());
        }
    ...
    const ManagerStore = (() => {
        ...
        async function loadStoreSpecificData(storeId) {
            try {
                ...
                // ===== 2. โหลดข้อมูลจาก API =====
                // ข้อมูลพื้นฐานจาก API ถูกโหลดมาแล้วใน init()
                // ที่นี่เราแค่ตรวจสอบว่ามีข้อมูลเฉพาะร้านใน localStorage ที่ต้องเอามาทับหรือไม่
                if (savedSettings) {
                    try {
                        const storeSpecificSettings = JSON.parse(savedSettings);
                        Object.assign(appData.shopSettings, storeSpecificSettings);
                        console.log('✅ Overlaid store settings from localStorage');
                    } catch (_) {}
                }
                // ===== END FIX =====
