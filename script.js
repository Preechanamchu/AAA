document.addEventListener('DOMContentLoaded', () => {
    ...
    const createConfirmOrderHTML = (orderNumber) => {
        ...
        for (const catId in itemsByCategory) {
            ...
            html += `<div class="summary-line">
                        <span>${catName}:</span>
                        <span>${totalQuantity} ${lang === 'th' ? 'ชิ้น' : 'pcs'}</span>
                    </div>`;
    ...
    window.addCoinToCart = (id) => {
        ...
        renderOrderSummary(); 
        saveCartItems();
        Notify.success('เพิ่มเหรียญแล้ว', `${pkg.coins.toLocaleString()} 💰 ถูกเพิ่มลงบัญชีสั่งซื้อ`);
    };
        ...
        for(const [level, pkgs] of Object.entries(levelGroups)) {
            ...
            groupDiv.innerHTML = `
                ...
                <div class="coin-package-list" style="display: flex; flex-direction: column; gap: 10px;">
                    ${pkgs.map(pkg => `
                        <div class="coin-package-item" style="display: flex; justify-content: space-between; align-items: center; background: #fff; padding: 12px 15px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: 1px solid #f0f0f0;">
                            ...
                            <button class="btn btn-primary btn-small" onclick="addCoinToCart('${pkg.id}')" style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem;">เลือก</button>
                        </div>
                    `).join('')}
                </div>
            `;
    ...
    document.getElementById('coin-package-form').addEventListener('submit', async (e) => {
        ...
        if (editingCoinPkgId) {
            ...
            if (index !== -1) {
                appData.shopSettings.coinPackages[index] = { id: editingCoinPkgId, levelRange, coins, price };
                addLog('Coin Package Updated', `Range: ${levelRange}, Coins: ${coins}, Price: ${price}`);
            }
            editingCoinPkgId = null;
        } else {
            const newPkg = { id: 'coin_' + Date.now(), levelRange, coins, price };
    ...
    window.renderAdminCoinPackages = () => {
        ...
        if (!appData.shopSettings.coinPackages || appData.shopSettings.coinPackages.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" style="text-align: center; color: var(--text-color);">ไม่มีข้อมูลแพ็กเกจ</td></tr>';
            return;
        }
