document.addEventListener('DOMContentLoaded', () => {
            ...
            groupDiv.innerHTML = `
                ...
                <div class="coin-package-list" style="display: flex; flex-direction: column; gap: 10px;">
                    ${pkgs.map(pkg => {
                        ...
                        return `
                        <div class="coin-package-item ${isSelected ? 'coin-pkg-selected' : ''} ${isDisabled ? 'coin-pkg-disabled' : ''}" style="display: flex; justify-content: space-between; align-items: center; background: ${isSelected ? 'linear-gradient(135deg, #f0fff4, #e6ffed)' : '#fff'}; padding: 12px 15px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: ${isSelected ? '2px solid #27ae60' : '1px solid #f0f0f0'}; transition: all 0.3s ease; ${isDisabled ? 'opacity: 0.5;' : ''}">
                            <div class="coin-info" style="display: flex; align-items: center; gap: 10px;">
                                ${iconHtml}
                                <span style="font-weight: bold; color: #ffa502; font-size: 1.1rem; min-width: 90px;">${pkg.coins.toLocaleString()} 💰</span>
                                <span style="color: #666; font-size: 0.95rem;">${pkg.price.toLocaleString()} บาท</span>
                            </div>
                            ${buttonHtml}
                        </div>
                    `;
    ...
    window.renderVoucherCatalogFrontend = () => {
        ...
        container.style.cssText = 'width: 100%; grid-column: 1/-1; display: grid; grid-template-columns: repeat(2, 1fr); gap: 15px; padding-bottom: 20px;';
        ...
        packages.forEach(pkg => {
            ...
            const iconUrl = pkg.icon || '';
            const iconHtml = iconUrl ? `<img src="${iconUrl}" alt="voucher" style="width: 28px; height: 28px; border-radius: 6px; object-fit: cover; margin-right: 8px;">` : '';
            ...
            if (isSelected) {
                ...
                btnOrCheck = `<button class="btn btn-primary btn-small" onclick="addVoucherToCart('${pkg.id}')" style="margin: 0; padding: 5px 15px; border-radius: 20px; font-size: 0.9rem;">เลือก</button>`;
            }
            ...
            itemDiv.style.cssText = `
                ...
                border-radius: 12px; padding: 15px; transition: all 0.2s ease;
            ...
            itemDiv.innerHTML = `
                <div style="display: flex; align-items: center; flex: 1; gap: 10px;">
                    ...
                    <div style="display: flex; flex-direction: column; text-align: left;">
                        <span style="font-weight: bold; font-size: 1.1rem; color: #333; min-width: 70px;">${pkg.quantity.toLocaleString()} ใบ</span>
                        <span style="color: #e74c3c; font-weight: bold; font-size: 1rem;">${pkg.price.toLocaleString()} บาท</span>
                    </div>
                </div>
                ${btnOrCheck}
            `;
            ...
            diamondDiv.innerHTML = `
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 15px;">
                    ${sortedDiamonds.map(pkg => {
                        ...
                        return `
                        <div class="diamond-pkg-item" style="display: flex; justify-content: space-between; align-items: center; background: ${isSelected ? 'linear-gradient(135deg, #f0fff4, #e6ffed)' : '#fff'}; padding: 12px 15px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05); border: ${isSelected ? '2px solid #27ae60' : '1px solid #e0f7fa'}; transition: all 0.3s ease; ${isDisabled ? 'opacity: 0.6;' : ''}">
                            <div style="display: flex; align-items: center; gap: 10px;">
                                ${iconHtml}
                                <span style="font-weight: bold; color: #0093e9; font-size: 1.1rem; min-width: 90px;">${pkg.diamonds.toLocaleString()} 💎</span>
                                <span style="color: #666; font-size: 0.95rem;">${pkg.price.toLocaleString()} บาท</span>
                            </div>
                            ${buttonHtml}
                        </div>`;
            ...
            passDiv.innerHTML = `
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px;">
                    ${sortedPasses.map(pkg => {
                        ...
                        return `
                        <div class="farmpass-pkg-item" style="display: flex; flex-direction: column; background: ${isSelected ? '#eaffef' : '#fff'}; padding: 20px; border-radius: 15px; box-shadow: 0 5px 15px rgba(0,0,0,0.08); border: ${isSelected ? '2px solid #27ae60' : '1px solid #f0f0f0'}; transition: all 0.3s ease; align-items: center; text-align: center; ${isDisabled ? 'opacity: 0.6;' : ''}">
                            <div style="width: 60%; max-width: 180px;">${imgHtml}</div>
                            <h4 style="margin: 0 0 10px 0; color: #333; font-size: 1.15rem;">${pkg.name}</h4>
                            <div style="font-weight: bold; color: #ff5722; font-size: 1.3rem; margin-bottom: 15px;">${pkg.price.toLocaleString()} บาท</div>
