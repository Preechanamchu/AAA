document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('swap-catalog-btn').addEventListener('click', () => {
        isCatalogUpdating = !isCatalogUpdating;
        const productGrid = document.getElementById('product-grid');
        if (productGrid) {
            if (isCatalogUpdating) {
                // Search container hide
                const searchContainer = document.querySelector('.search-and-filter-container');
                if (searchContainer) searchContainer.style.display = 'none';

                renderCoinCatalogFrontend();

                document.getElementById('current-category-name').textContent = "เหรียญ";
                document.getElementById('cat-scroll-left').style.display = 'none';
                document.getElementById('cat-scroll-right').style.display = 'none';
                document.getElementById('category-tabs-container').style.display = 'none';
            } else {
                // Restore catalog
                // Search container restore
                const searchContainer = document.querySelector('.search-and-filter-container');
                if (searchContainer) searchContainer.style.display = '';

                document.getElementById('cat-scroll-left').style.display = '';
                document.getElementById('cat-scroll-right').style.display = '';
                document.getElementById('category-tabs-container').style.display = '';
