document.addEventListener('DOMContentLoaded', () => {
    ...
    const switchToCatalogMode = (page, skipTransition = false) => {
        catalogPage = page;
        const productGrid = document.getElementById('product-grid');
        ...
        const updateUI = () => {
            if (catalogPage === 1 || catalogPage === 2 || catalogPage === 3) {
                // Hide entire controls wrapper (search, filter, category tabs)
                if (controlsWrapper) {
                    if (!skipTransition) controlsWrapper.style.transition = 'opacity 0.3s ease, max-height 0.4s ease';
    ...
    document.getElementById('swap-catalog-btn').addEventListener('click', function() {
        // Increment and wrap rotation
        swapRotation += 360;
        this.style.transform = `rotate(${swapRotation}deg)`;
        
        // Cycle through catalog pages
        const nextMode = (catalogPage + 1) % 4;
        switchToCatalogMode(nextMode);
    });
    ...
    const init = async () => {
        ...
        // ===== START: Deep Link / Product Link Auto-Switch =====
        const urlParams = new URLSearchParams(window.location.search);
        const modeParam = urlParams.get('mode');
        const productIdParam = urlParams.get('id');

        if (modeParam === 'coin' || modeParam === 'coins') {
            catalogPage = 1;
        } else if (modeParam === 'diamond' || modeParam === 'diamonds' || modeParam === 'pass') {
            catalogPage = 2;
        } else if (modeParam === 'voucher' || modeParam === 'vouchers') {
            catalogPage = 3;
        } else if (productIdParam) {
            // Check if it's a special package ID
            if (productIdParam.startsWith('coin_')) catalogPage = 1;
            else if (productIdParam.startsWith('diamond_') || productIdParam.startsWith('pass_')) catalogPage = 2;
            else if (productIdParam.startsWith('voucher_')) catalogPage = 3;
        }
        // ===== END: Deep Link / Product Link Auto-Switch =====
        ...
        // Handle initial catalog rendering
        if (catalogPage !== 0) {
            switchToCatalogMode(catalogPage, true); // True to skip transition on initial load
