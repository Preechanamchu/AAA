document.addEventListener('DOMContentLoaded', () => {
            ...
            passDiv.innerHTML = `
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 20px;">
                    ${sortedPasses.map(pkg => {
                        ...
                        let buttonHtml = isSelected 
                            ? `<button class="btn btn-block" disabled style="padding: 10px; border-radius: 8px; font-size: 1.3rem; background: linear-gradient(135deg, #27ae60, #2ecc71); color: #fff; border: none; font-weight: bold; cursor: default; box-shadow: 0 3px 10px rgba(39,174,96,0.4); animation: coinCheckPop 0.3s ease;">✓</button>`
                            : (isDisabled 
    ...
    const init = async () => {
        ...
        // ===== START FIX: โหลดข้อมูลร้านค้าให้เสร็จก่อน loader หาย =====
        // เช็คว่ามี ?store= ใน URL หรือไม่ ถ้ามีให้ await checkUrlForStore() ก่อน
        // เพื่อให้ข้อมูล เหรียญ / เพชร / บัตรกำนัล พร้อมแสดงทันทีเมื่อ loader ปิด
