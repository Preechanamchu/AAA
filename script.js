document.addEventListener('DOMContentLoaded', () => {
    ...
    const createConfirmOrderHTML = (orderNumber) => {
        ...
        html += `<h3 style="text-align:center;">${appData.shopSettings.shopName}</h3>`;
        if (orderNumber) {
            html += `<div style="text-align:center; margin-bottom: 10px; font-weight: bold; color: var(--primary-color);">
                        ${lang === 'th' ? 'เลขที่ออเดอร์' : 'Order No.'}: ${orderNumber}
                    </div>`;
        }
        html += '<hr style="border: 0.5px dashed var(--border-color); margin: 10px 0;">';

        Object.keys(itemsByCategory).forEach(catId => {
            const categoryData = itemsByCategory[catId];
            const catName = (lang === 'en' && categoryData.name_en) ? categoryData.name_en : categoryData.name;
            html += `<h4 class="order-summary-cat-title" style="margin-top: 15px;">📋 ${catName}</h4>`;
    ...
    const handleOrderAction = (isConfirm) => {
        ...
        if (isConfirm) {
            ...
            // Use HTML for visual display in modal
            orderDetails.innerHTML = createConfirmOrderHTML(orderNumber);
            
            // Still generate the Text version and store it for clinical/clipboard use if needed
            // But usually we generate it when the copy button is clicked
            // ===== END: MODIFICATION =====
